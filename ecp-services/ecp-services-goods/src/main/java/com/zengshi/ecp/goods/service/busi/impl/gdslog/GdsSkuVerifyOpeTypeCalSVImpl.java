 ...
 * @since JDK 1.6 
 */
public class GdsSkuVerifyOpeTypeCalSVImpl implements ICalOperationTypeSV {
			...
			if(StringUtil.isNotBlank(reqDTO.getOperateType())){
				...
				case GdsConstants.GdsInfo.GDS_STATUS_DELETE:
					if(ArrayUtils.isNotEmpty(reqDTO.getIds())){
						type = OperationType.SKU_BATCH_DELETE_VERIFY;
					}else{
						type = OperationType.SKU_DELETE_VERIFY;
					}
					break;
				// 上架审核
				case GdsConstants.GdsInfo.GDS_STATUS_ONSHELVES:
					if(ArrayUtils.isNotEmpty(reqDTO.getIds())){
						type = OperationType.SKU_BATCH_ON_SHELVES_VERIFY;
					}else{
						type = OperationType.SKU_ON_SHELVES_VERIFY;
					}
					break;
				default:
					break;
