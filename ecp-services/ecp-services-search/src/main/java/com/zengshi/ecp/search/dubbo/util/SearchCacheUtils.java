...
import com.zengshi.ecp.search.dubbo.interfaces.*;
...
import com.zengshi.ecp.search.dubbo.search.util.SecConfigConfigurationWatcher;
...
public class SearchCacheUtils {
    ...
    public static Map<String, SecConfigRespDTO> getSecConfigReqDTOMap() throws SearchException {
        ...
        if(cacheSecConfigRespDTOMap==null){
            ...
            if (CacheUtil.exists(CACHE_KEY_SECCONFIGMAP)) {
                Map<String, String> map = com.zengshi.paas.utils.CacheUtil
    ...
    public static String getSecZkStr() throws SearchException {
        ...
        if(cacheZkHost==null){
            ...
            // 缓存数据是最新的
            if (CacheUtil.exists(CACHE_KEY_SECZKSTR) && StringUtil.isNotEmpty(CacheUtil.getItem(CACHE_KEY_SECZKSTR))) {
                ...
                if (!CacheUtil.exists(CACHE_KEY_SECZKSTR) || StringUtil.isEmpty(CacheUtil.getItem(CACHE_KEY_SECZKSTR))) {
                    synchronized (SearchCacheUtils.class) {
                        if (!CacheUtil.exists(CACHE_KEY_SECZKSTR) || StringUtil.isEmpty(CacheUtil.getItem(CACHE_KEY_SECZKSTR))) {
                            // 缓存失效或第一次初始化缓存，重新查表
    ...
    public static HashMap<String,String> getSecArgsMap(){
        ...
        if(cacheSecArgsMap==null){
            ...
            // 缓存数据是最新的
            if (CacheUtil.exists(CACHE_KEY_SECARGSMAP) && StringUtil.isNotEmpty(CacheUtil.getItem(CACHE_KEY_SECARGSMAP))) {

                Map<String, String> map = com.zengshi.paas.utils.CacheUtil
                        .getHashMap(CACHE_KEY_SECARGSMAP);
                ...
                if (!CacheUtil.exists(CACHE_KEY_SECARGSMAP) || StringUtil.isEmpty(CacheUtil.getItem(CACHE_KEY_SECARGSMAP))) {
                    synchronized (SearchCacheUtils.class) {
                        if (!CacheUtil.exists(CACHE_KEY_SECARGSMAP) || StringUtil.isEmpty(CacheUtil.getItem(CACHE_KEY_SECARGSMAP))) {
                            // 缓存失效或第一次初始化缓存，重新查表
