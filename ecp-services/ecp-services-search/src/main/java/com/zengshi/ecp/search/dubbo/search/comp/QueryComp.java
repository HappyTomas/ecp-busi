...
import com.zengshi.ecp.search.dubbo.search.QueryBuilderResp;
...
import com.zengshi.ecp.search.dubbo.search.support.*;
import com.zengshi.ecp.search.dubbo.search.support.RedirectSupport.RedirectResult;
 ...
 * Description: 通用搜索接口<br>
 * Date:2016年1月13日下午12:46:25  <br>
...
public class QueryComp extends BaseComp {
    ...
    public static <T> SearchResult<T> search(EBinderType binderType, SearchParam searchParam,
        ...
        LogUtil.info(MODULE, "ResultBinder实例：【"+binderType.getBinderType()+"】");
        ...
        LogUtil.info(MODULE, "getAndValidateSecConfig耗时：【"+(endTime-startTime)+"ms】");
        ...
        try {
            ...
            solrServer = SearchUtils.getSolrServer(searchParam.getCollectionName(),true);
            endTime=System.currentTimeMillis();
            LogUtil.info(MODULE, "SearchUtils.getSolrServer耗时：【"+(endTime-startTime)+"ms】");
            ...
            endTime=System.currentTimeMillis();
            LogUtil.info(MODULE, "queryBuilderResp构造耗时：【"+(endTime-startTime)+"ms】");

            //发起请求
            startTime=System.currentTimeMillis();
            queryResp = solrServer.query(queryBuilderResp.getSolrQuery(), METHOD.POST);
            endTime=System.currentTimeMillis();
            LogUtil.info(MODULE, "HTTP请求响应耗时-SolrServer.query耗时：【"+(endTime-startTime)+"ms】");
        ...
        if (solrDocList != null && solrDocList.size() > 0) {
            ...
            try {
                ...
                endTime=System.currentTimeMillis();
                LogUtil.info(MODULE, "返回结果解析、对象和属性拷贝耗时：【"+(endTime-startTime)+"ms】");
            } catch (ReusltBindingException e) {
                LogUtil.error(MODULE, TEXT, e);
    ...
    private static QueryBuilderResp query(SecConfigRespDTO currSecConfigRespDTO,
        ...
        if(StringUtils.equals(currSecConfigRespDTO.getConfigIfMultilan(),SearchConstants.STATUS_1)||StringUtils.equals(currSecConfigRespDTO.getConfigIfMultilan(),SearchConstants.STATUS_2)){
            langFieldTypes=langAnalysis(searchParam.getKeyword());

            //结果中搜的多语言支持
            langFieldTypes=SearchInResultSupport.mergeSearchInResultLang(langFieldTypes,searchParam);
        }
        ...
        if(!queryAll&&StringUtils.equals(currSecConfigRespDTO.getConfigIfMultilan(),SearchConstants.STATUS_1)){
            ...
            //引擎内自动判断语言环境（可能识别匹配多种语言）
            if(CollectionUtils.isNotEmpty(langFieldTypes)){
                StringBuffer sb=new StringBuffer();
                for(ELang lang:langFieldTypes){
                    if(StringUtils.isNotBlank(sb.toString())) sb.append(" "+EOperator.OR.getName()+" ");
                    sb.append(SearchConstants.FIELD_LANG_FIELD_TYPE +SearchConstants.COLON+lang.getLang());
                }
                solrQuery.addFilterQuery(sb.toString());
            }
        }
        ...
        solrQuery=SearchInResultSupport.searchInResult(solrQuery, searchParam);
        ...
        LogUtil.info(MODULE, "查询请求参数：【"+searchParam.toString()+"】");
        LogUtil.info(MODULE, "查询转换模式：【"+currSecConfigRespDTO.getConfigQueryOp()+"】");
        LogUtil.info(MODULE, "客户端强制指定语言参数langs：【");
        if(CollectionUtils.isNotEmpty(searchParam.getLangs())){
            for(ELang lang:searchParam.getLangs()){
        ...
        LogUtil.info(MODULE, "】");
        LogUtil.info(MODULE, "文本分析语言字段类型结果langFieldTypes：【");
        if(CollectionUtils.isNotEmpty(langFieldTypes)){
            for(ELang lang:langFieldTypes){
                LogUtil.info(MODULE,lang.getLang());
            }
        ...
        LogUtil.info(MODULE, "最终查询串：【"+solrQuery.getQuery()+"】");
        LogUtil.info(MODULE, "最终排序串：【"+solrQuery.get(CommonParams.SORT)+"】");
        LogUtil.info(MODULE, "默认检索字段（df）：【"+solrQuery.get(CommonParams.DF)+"】");
        LogUtil.info(MODULE, "最终Filter查询串：【");
        ...
        LogUtil.info(MODULE, "可设置权重字段pf："+solrQuery.get("pf"));
        LogUtil.info(MODULE, "字段权重串qf："+solrQuery.get("qf"));
        LogUtil.info(MODULE, "函数查询Function Query,bf："+solrQuery.get("bf"));
        LogUtil.info(MODULE, "boost："+solrQuery.get("boost"));
        LogUtil.info(MODULE,"===================高亮控制参数===================");
        LogUtil.info(MODULE, "高亮控制参数hl："+solrQuery.get("hl"));
        LogUtil.info(MODULE, "高亮控制参数hl.fl：【");
        String fls[]=solrQuery.getParams("hl.fl");
        if(fls!=null&&fls.length>0){
            for(String fl:fls){
                LogUtil.info(MODULE,fl);
            }
        }
        LogUtil.info(MODULE, "】");
        LogUtil.info(MODULE, "高亮控制参数hl.useFastVectorHighlighter："+solrQuery.get("hl.useFastVectorHighlighter"));
        LogUtil.info(MODULE, "高亮控制参数hl.fragsize："+solrQuery.get("hl.fragsize"));
        LogUtil.info(MODULE, "高亮控制参数hl.snippets："+solrQuery.get("hl.snippets"));
        LogUtil.info(MODULE, "高亮控制参数hl.boundaryScanner："+solrQuery.get("hl.boundaryScanner"));
        LogUtil.info(MODULE, "高亮控制参数hl.bs.type："+solrQuery.get("hl.bs.type"));
        LogUtil.info(MODULE, "高亮控制参数hl.bs.maxScan："+solrQuery.get("hl.bs.maxScan"));
        LogUtil.info(MODULE, "高亮控制参数hl.bs.chars："+solrQuery.get("hl.bs.chars"));
        LogUtil.info(MODULE, "高亮控制参数hl.bs.language："+solrQuery.get("hl.bs.language"));
        LogUtil.info(MODULE, "高亮控制参数hl.bs.country："+solrQuery.get("hl.bs.country"));
