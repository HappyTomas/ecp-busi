...
import com.zengshi.ecp.search.dubbo.search.util.EQOp;
...
public class SearchParam {
    ...
    /**
     * 语种，用于多语言查询（支持设置多个）。此参数仅仅用于当客户端需要强制指定检索语言的时候使用（如界面提供检索语言参数选择）。否则语言环境由引擎内自动判断（可能识别匹配多种语言）
     * 区分langFieldType。lang和langFieldType是需要共存的。
     * 1、界面没有设置语种，则进行字段语言分析（langFieldType）
     * 2、界面语言选择了英文（特例，又如CJK），则不能直接使用texten进行搜索，因为目前中英文统一使用textcn字段类型（因此才需要区分lang和langFieldType）。
     */
