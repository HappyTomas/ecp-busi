...
import com.zengshi.ecp.order.facade.interfaces.eventual.IOrdRemoveCouponSV;
...
public class OrdRemoveCouponSVImpl implements IOrdRemoveCouponSV {
    ...
    private static final String MODULE = OrdRemoveCouponSVImpl.class.getName();
    ...
    public void joinTransaction(JSONObject message, TransactionStatus status, String transactionName) {
        try {
            ...
            LogUtil.info(MODULE,"取消订单入参OrdRemoveCouponSVImpl============="+message.toString());
            
            dealMethod(rOrderDetailsRequest);
        } catch (BusinessException be) {
            LogUtil.error(MODULE, "取消订单优惠券域接口处理业务异常", be);
            status.setRollbackOnly();
            throw new BusinessException(be.getErrorCode());
        } catch (Exception e) {
            LogUtil.error(MODULE, "取消订单优惠券域接口处理系统异常", e);
            status.setRollbackOnly();
            throw new BusinessException(MsgConstants.OtherSysMsgCode.CALL_COUP_SERVER_343000);
        }
    }
    ...
    public void dealMethod(ROrderDetailsRequest rOrderDetailsRequest) {
        ...
        LogUtil.info(MODULE,"调用优惠券域入参"+JSON.toJSONString(rOrdCartsCommRequest));
        //优惠券补偿
