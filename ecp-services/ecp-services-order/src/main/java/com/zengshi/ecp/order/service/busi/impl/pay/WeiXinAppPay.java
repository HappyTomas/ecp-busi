 ...
 * @since JDK 1.7
 */
public class WeiXinAppPay extends DefaultPayWay {
	public static final String module = WeiXinAppPay.class.getName();
	public final static String payWay = PayWayEnum.getPayWayByImplClass(WeiXinAppPay.class);
	private final static String TRAED_TYPE = "APP";
...
/*		ROrdPayRelReq rOrdPayRelReq = new ROrdPayRelReq();
				...
				if (OrdSubList != null && OrdSubList.size() > 0) {
					for (OrdSub ordSub : OrdSubList) {
						if (sb.length() > 128) {
							...
							sb.append(ordSub.getGdsName()).append("*");
						}
					}
		...
		weChatRe.setNonce_str(nonceStr);
		...
		LogUtil.info(module, "微信App支付预下单加签前的数据为"+data);
		...
		LogUtil.info(module, "微信App支付预下单加签后的数据为"+JSONObject.toJSONString(weChatRe));
		postData2WeChat(pr.getActionUrl(), weChatRe, map); // 向微信提交数据

		LogUtil.info(module, "微信App支付预下单成功后返回的数据为"+JSONObject.toJSONString(map));
		Map<String, String> WeixinJSBridgeMap = new HashMap<String, String>();
		WeixinJSBridgeMap.put("appid", pay_private_key);
		WeixinJSBridgeMap.put("timestamp", String.valueOf(new Date().getTime()).substring(0, 10));
/*		WeixinJSBridgeMap.put("noncestr", UUID.randomUUID().toString().replace("-", ""));*/
		WeixinJSBridgeMap.put("noncestr", nonceStr);
		WeixinJSBridgeMap.put("package", "Sign=WXPay");
		WeixinJSBridgeMap.put("prepayid", map.get("prepay_id"));
		WeixinJSBridgeMap.put("partnerid", payMercCode);
		String WeixinJSBridgeData = WeChatPayRequest.createLinkString(WeixinJSBridgeMap);
		LogUtil.info(module, "微信App支付正式支付加签前的数据为"+WeixinJSBridgeData);
		String WeixinJSBridgeSign = weChatRe.buildSign(new String[] { WeixinJSBridgeData, signKey });
		LogUtil.info(module, "微信App支付正式支付加签后的数据为"+WeixinJSBridgeSign);		
		WeixinJSBridgeMap.put("sign", WeixinJSBridgeSign);
	...
	public Map<String, String> paymentCallback(Map<String, String> responseResultMap) throws Exception {
		LogUtil.info(module, "微信App支付回调开始" + PayHelper.toJsonStr(responseResultMap));
		...
		LogUtil.info(module, "订单" + orderId + "微信App支付回调结束" + PayHelper.toJsonStr(result));
		return result;
	}
