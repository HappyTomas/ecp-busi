 ...
 * @since JDK 1.7
 */
public class WeChatPay extends DefaultPayWay {
	public static final String module = WeChatPay.class.getName();
	public final static String payWay = PayWayEnum.getPayWayByImplClass(WeChatPay.class);
	private final static String TRAED_TYPE = "JSAPI";
...
/*		ROrdPayRelReq rOrdPayRelReq = new ROrdPayRelReq();
		...
		if (list != null && list.size() >= 1) {
			for (ROrdPayRelResp resp : list) {
				...
				if (OrdSubList != null && OrdSubList.size() > 0) {
					...
					if(sb.length()>= 100){
						subject=sb.toString().substring(0, 99);
					}else{
		...
		weChatRe.setOpenid(extendProps.get("openid"));
	...
	public Map<String, String> paymentCallback(Map<String, String> responseResultMap) throws Exception {
		LogUtil.info(module, "微信支付回调开始" + PayHelper.toJsonStr(responseResultMap));
...
/*			String appid = response.getAppid();
			String mch_id = response.getMch_id();
			...
			sParaNew = WeChatPayRequest.paraFilter(weChat.toMap());
			preSignStr = WeChatPayResponse.createLinkString(sParaNew);
			sign = weChat.buildSign(new String[] { preSignStr, payShopCfg.getKeyName() });
			weChat.setSign(sign);
			PayRequestData pr = new PayRequestData();
			pr.setActionUrl(payWayBean.getPayQueryUrl());
			postData2WeChat(pr.getActionUrl(), weChat, result);*/
			// 查询返回的订单流水号是否已支付成功
/*			if (result.get(OrdConstants.PayWayWeChat.RET_KEY).equals(OrdConstants.PayWayWeChat.RET_VALUE)
					&& result.get(OrdConstants.PayWayWeChat.TRADE_STATE).equals(OrdConstants.PayWayWeChat.RET_VALUE)) {*/
		...
		LogUtil.info(module, "订单" + orderId + "微信公众号支付回调结束" + PayHelper.toJsonStr(result));
		return result;
	}
	...
	public static void main(String[] args) throws Exception {
		 ...
		 * ""); WeChatPayRequest weChatRe = new WeChatPayRequest();
		 * weChatRe.setAppid("wxb7db516174566d9c");
		 * weChatRe.setMch_id("1249747901");
		 * weChatRe.setNonce_str("cbb2935c9544495e985b7ccc31733963");
		 * weChatRe.setBody("%E6%B5%8B%E8%AF%95001*1");
		 * weChatRe.setOut_trade_no("RW16041400002015");
		 * weChatRe.setTotal_fee(12476);
		 * weChatRe.setSpbill_create_ip("10.37.238.37");
		 * weChatRe.setNotify_url("10.37.238.37");
		 * weChatRe.setTrade_type("NATIVE"); Map<String, String> map =
		 * WeChatPayRequest.paraFilter(weChatRe.toMap()); String data =
		 * WeChatPayRequest.createLinkString(map); String sign =
		 * weChatRe.buildSign(new String[] { data,
		 * "guang1xi9hua8shvsens2comcjxwcf20" }); weChatRe.setSign(sign);
		 * System.out.println(uuid); System.out.println(sign); map =
		 * WeChatPayRequest.paraFilter(weChatRe.toMap()); String xml =
		 * WeChatPayRequest.createXml(map); System.out.println(xml); String
		 * content = ""; data = WeChatPayRequest.createLinkString(map); URL url
		 * = new URL("https://api.mch.weixin.qq.com/pay/unifiedorder");
		 ...
		 * connection.setRequestProperty("Charsert", "UTF-8"); PrintWriter out =
		 * null; BufferedReader in = null; out = new
		 * PrintWriter(connection.getOutputStream()); // 发送请求参数 out.print(xml);
		 * // flush输出流的缓冲 out.flush(); in = new BufferedReader(new
		 * InputStreamReader(connection.getInputStream())); String line; while
		 * ((line = in.readLine()) != null) { content += line; }
		 * System.out.println(content); in.close(); out.close();
		 * 
		 * String xml =
		 * "<xml><return_code><![CDATA[SUCCESS]]></return_code><return_msg><![CDATA[OK]]></return_msg><appid><![CDATA[wxb7db516174566d9c]]></appid><mch_id><![CDATA[1249747901]]></mch_id><nonce_str><![CDATA[QcFubKIDB9KO9oO6]]></nonce_str><sign><![CDATA[98552F784EE41A44B44D1860EE541F04]]></sign><result_code><![CDATA[SUCCESS]]></result_code><prepay_id><![CDATA[wx20160415172042e5425abee20887860846]]></prepay_id><trade_type><![CDATA[NATIVE]]></trade_type><code_url><![CDATA[weixin://wxpay/bizpayurl?pr=Lw3DdYy]]></code_url></xml>"
		 * ; Map<String, String> map = new HashMap<String, String>();
		 * parseXml2Map(xml,map); System.out.println(map); WeChatScanCodePay pay
		 * = new WeChatScanCodePay(); Map<String, String> extendProps = new
		 * HashMap<String, String>(); extendProps.put("clientIp", "127.0.0.1");
		 * RPayRefundRequest request = new RPayRefundRequest();
		 * request.setPayWay("9005"); request.setBackId(100L);
		 * request.setRefundAmount(1L); request.setOperId("12345");
		 * request.setOrderId("RW16041700002065"); RPayRefundResponse result =
		 * pay.refund(request, extendProps); System.out.println(result);
		 **/
