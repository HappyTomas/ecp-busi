 ...
 * @since JDK 1.6 
 */
public class ScoreToOrderOtherSVImpl implements IScoreToOrderOtherSV {
    
    private  static final String MODULE = ScoreToOrderOtherSVImpl.class.getName();   
    ...
    public boolean saveScoreUseForOrderRefund(OrderBackSubReqDTO reqDto) throws BusinessException {
        ...
        for (ScoreExchangeResDTO scoreEx: scoreExList) {
            ...
            req.setScore(scoreEx.getScoreUsing() * reqDto.getScale()/1000000);//精度丢失，相当于向下取整
            
            /*2016-08-08 huangxl5配合可部分退货改造：此处逻辑改为，当为最后一笔时，取子订单获得的积分减去已返还的积分*/
            if (reqDto.isLastFlag()) {
            	long backScore = this.getLastScoreForUseBack(scoreEx);//子订单的最后一个退货时，返还当时使用的剩余积分
            	req.setScore(scoreEx.getScoreUsing() - backScore);
            }
            ...
            /*2016-08-08 huangxl5配合可部分退货改造：此处逻辑改为，当为最后一笔时，才更新该条记录状态为1*/
            if (reqDto.isLastFlag()) {
    ...
    /**
     * 
     * getLastScoreForUseBack:(查询子订单因退货或退款，已经返还的积分). <br/> 
     * 
     * @author huangxl 
     * @param scoreEx
     * @return 
     * @since JDK 1.6
     */
    private long getLastScoreForUseBack(ScoreExchangeResDTO scoreEx) {
    	
        PageResponseDTO<ScoreSourceResDTO> spage =  null;
        ScoreSourceReqDTO sourceReq = new ScoreSourceReqDTO();
        sourceReq.setStaffId(scoreEx.getStaffId());
        sourceReq.setOrderId(scoreEx.getOrderId());
        sourceReq.setSubOrderId(scoreEx.getSubOrderId());
        sourceReq.setPageNo(0);
        sourceReq.setBackFlag(StaffConstants.Score.SCORE_BACK_FLAG_2);//查询子订单回退过的积分
    ...
    /**
     * 
     * getLastScoreForAddBack:(因退货退款，子订单获得的积分已退了多少). <br/> 
     * 
     * @author huangxl5 
     * @param sourceRes
     * @return 
    ...
    private long getLastScoreForAddBack(ScoreSourceResDTO sourceRes) {
        ...
        backReq.setSubOrderId(sourceRes.getSubOrderId());
        backReq.setPageNo(0);
    ...
    public Long saveScoreFrozenForOrderBackSub(OrderBackSubReqDTO req) throws BusinessException {
        ...
        if (StringUtil.isNotBlank(req.getOrderId())) {
            ...
            sourceReq.setStaffId(req.getStaffId());
            sourceReq.setOrderId(req.getOrderId());
            sourceReq.setSubOrderId(req.getSubOrderId());
            sourceReq.setPageNo(0);
            spage = scoreInfoSV.listScoreSource(sourceReq);
            if(spage == null || CollectionUtils.isEmpty(spage.getResult())) {
                LogUtil.info(MODULE, "根据用户以及订单信息：["+req.toString()+"]没有查询到对应的积分来源记录，无需进行业务操作。");
        ...
        for (ScoreSourceResDTO sourceRes : spage.getResult()) {
           ...
           if (StringUtil.isNotBlank(sourceRes.getSubOrderId()) && sourceRes.getSubOrderId().equals(req.getSubOrderId())) {
               long score = 0;
               /*2016-08-08 huangxl5配合可部分退货改造：非最后一笔子订单退货时，乘以比例，最后一笔时做减法*/
               if (req.isLastFlag()) {
            	   score = sourceRes.getScore();
               } else {
            	   score = sourceRes.getScore() * req.getScale() / 1000000;
               }
               subOrderScore += score;
           }
        }
        if (req.isLastFlag()) {
        	...
        	sourceRes.setSubOrderId(req.getSubOrderId());
        	sourceRes.setStaffId(req.getSiteId());
        	subOrderScore = subOrderScore - this.getLastScoreForAddBack(sourceRes);
        }
    ...
    public boolean saveScoreAddForOrderBackSub(OrderBackSubReqDTO req) throws BusinessException {
        ...
        for (ScoreSourceResDTO sourceRes : spage.getResult()) {
           ...
           //子订单获得的积分
           if (StringUtil.isNotBlank(sourceRes.getSubOrderId()) && sourceRes.getSubOrderId().equals(req.getSubOrderId())) {
               /*2016-08-08 huangxl5配合可部分退货改造：非最后一笔子订单退货退款时，乘以比例，为最后一笔时，做减法*/
        	   long subScoreBack = 0L;
        	   if (req.isLastFlag()) {
        		   //子订单获得的积分减去因部分退货已经扣掉的积分
        		   //subScoreBack = sourceRes.getScore() - this.getLastScoreForAddBack(sourceRes);
        		   subScoreBack += sourceRes.getScore();
        	   } else {
        		   subScoreBack = sourceRes.getScore() * req.getScale() / 1000000;//精度丢失，相当于向下取整
        	   }
        	   subOrderScore += subScoreBack;
               ...
               exchangeReq.setOrderId(sourceRes.getOrderId());
               exchangeReq.setSubOrderId(sourceRes.getSubOrderId());
               exchangeReq.setScoreUsing(subScoreBack);//积分
               exchangeReq.setSiteId(sourceRes.getSiteId());//站点id
               exchangeReq.setCreateStaff(req.getStaffId());
               //订单入参：0，退款   1，退货
               ...
               exchangeReq.setBackFlag(StaffConstants.Score.SCORE_BACK_FLAG_2);
               exchangeReq.setExchangeMode(StaffConstants.Score.SCORE_EXCHANGE_MODE_2);//退货，退款
               //为最后一笔时，只做一次更新操作，即子订单送的积分与促销的合并为一条记录
               if (req.isLastFlag()) {
            	   if (count == spage.getCount()) {
            		   subOrderScore = subOrderScore - this.getLastScoreForAddBack(sourceRes);
            		   exchangeReq.setScoreUsing(subOrderScore);
               ...
               log.setFreezeScore(scoreInfo.getScoreFrozen());//操作前，冻结积分
               log.setUsedScore(scoreInfo.getScoreUsed());//操作前，已使用积分
               log.setScore(subScoreBack);//本次操作的积分
               log.setStaffId(req.getStaffId());//用户ID
               //订单入参：0，退款   1，退货
               if ("0".equals(req.getRefundOrBack())) {
               ...
               /*2016-08-08 huangxl5配合可部分退货改造：当为子订单的最后一笔退货时，才变更状态*/
               if (req.isLastFlag()) {
            	   ScoreSourceReqDTO sourceReq = new ScoreSourceReqDTO();
                   sourceReq.setBackFlag(StaffConstants.Score.SCORE_BACK_FLAG_1);
                   sourceReq.setId(sourceRes.getId());
    ...
    public boolean saveScoreAddForOrderBackMain(OrderBackMainReqDTO<OrderBackSubReqDTO> req) throws BusinessException {
        ...
        if ("0".equals(req.getRefundOrBack())) {
        	mainExchange.setRemark("订单退款，扣减该主订单获得的积分。扣减比例为：" + req.getScale() / 10000 + "%");
        } else {
        	mainExchange.setRemark("订单退货，扣减该主订单获得的积分。扣减比例为：" + req.getScale() / 10000 + "%");
        }
        ...
        log.setStaffId(req.getStaffId());//用户ID
        if ("0".equals(req.getRefundOrBack())) {
        	log.setRemark("订单退款，主订单上之前冻结的积分解冻，并扣除。扣减比例为：" + req.getScale() / 10000 + "%");//备注
        } else {
        	log.setRemark("订单退货，主订单上之前冻结的积分解冻，并扣除。扣减比例为：" + req.getScale() / 10000 + "%");//备注
        }
        log.setOrderId(req.getOrderId());//订单编码
    ...
    public long selTotalScoreByBackOrderSub(OrderBackSubReqDTO req) throws BusinessException {
        ...
        for (ScoreSourceResDTO sourceRes : spage.getResult()) {
           ...
           if (StringUtil.isNotBlank(sourceRes.getSubOrderId()) && sourceRes.getSubOrderId().equals(req.getSubOrderId())) {
               ...
               /*2016-08-08 huangxl5配合可部分退货改造：非最后一笔子订单退货时，乘以比例，最后一笔时做减法*/
               if (req.isLastFlag()) {
            	   //score = sourceRes.getScore() - this.getLastScoreForAddBack(sourceRes);
            	   score = sourceRes.getScore();
               } else {
            	   score = sourceRes.getScore() * req.getScale() / 1000000;
               }
