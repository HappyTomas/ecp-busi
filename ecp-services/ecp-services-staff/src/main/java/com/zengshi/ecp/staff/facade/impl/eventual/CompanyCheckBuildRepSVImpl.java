...
import com.zengshi.ecp.staff.facade.interfaces.eventual.ICompanyCheckBuildRepSV;
 ...
 * @since JDK 1.7
 */
@Service("companyCheckTxBuildRepSV")
public class CompanyCheckBuildRepSVImpl implements ICompanyCheckBuildRepSV {

    @Resource
    private IGdsStockRSV gdsStockRSV;
    
    private static final String MODULE = CompanyCheckBuildRepSVImpl.class.getName();
    ...
    public void joinTransaction(JSONObject message, TransactionStatus status, String transctionName) {
        ...
        try {
            ...
            LogUtil.info(MODULE, "=========审核企业通过，同时创建店铺仓库的分布式事务，新增仓库子事务开始========");
            StockRepMainReqDTO repMain = new StockRepMainReqDTO();
            repMain.setStaffId(res.getStaffId());//操作人
            StockRepReqDTO rep = new StockRepReqDTO();
            rep.setRepName(res.getShopName() + "的仓库");//仓库名称
            rep.setShopId(res.getShopId());
            rep.setCompanyId(res.getCompanyId());//企业ID
            rep.setCodeType(GdsConstants.GdsStock.STOCK_CODE_TYPE_SELLER);//卖家仓库
            rep.setRepType(GdsConstants.GdsStock.STOCK_REP_TYPE_PUBLIC);//卖家共仓
            rep.setRemark("店铺新增");//备注
            repMain.setStockRepDTO(rep);
            gdsStockRSV.addStockRep(repMain);
