 ...
 * @since JDK 1.7
 */
public class DataAuthControlRSVImpl extends AbstractRuleOfDataAuth implements IRuleOfDataAuth {
	...
	public List<RuleObject> getRuleObjects(String funcCode, long staffId) {
		...
		for (Long staffRoleId : listStaffRole) {//数据功能下的明细
			...
			dataAuthCriteria.createCriteria().andEnabledEqualTo(StaffConstants.PublicParam.STATUS_ACTIVE)//仅“启用”状态
				.andIdIn(listRuleId).andAuthSrcEqualTo(StaffConstants.DataAuth.AUTH_SRC_ROW).andFuncIdEqualTo(dataFuncResDto.getId());
			...
			for (DataAuth dataAuth : listDataAuth) {//规则下的明细
				...
				if(pageDataRule.getCount()>0){
					for (DataRuleResDTO dataRule : pageDataRule.getResult()) {
						...
						RuleObject ro = new RuleObject(name,field,op,value,isOr,hasLeft,hasRight);
						
						if("between".equals(dataRule.getOperChar())||"notBetween".equals(dataRule.getOperChar())){
							String[] arrayS = value.split(",");
							ro = new RuleObject(name,field,op,arrayS,isOr,hasLeft,hasRight);
						}
						if("in".equals(dataRule.getOperChar())||"notIn".equals(dataRule.getOperChar())){
							List<String> listS = new ArrayList<String>();
							listS = Arrays.asList(value.split(","));
							ro = new RuleObject(name,field,op,listS,isOr,hasLeft,hasRight);
						}
						
						String valueClazz = itemDTO.getValueType();
						super.convertValueClass(ro, valueClazz);
						list.add(ro);
					}
				}
	...
	public boolean judgeAuthOfCurrentUser(String funcCode, long staffId) {
		...
		LogUtil.info("DataAuthControlRSVImpl","方法：judgeAuthOfCurrentUser-->规则权限用户id改为从DTO里取:" + staffId);
		dataFuncReqDto.setCode(funcCode);
		dataFuncReqDto.setPageNo(0);//查全集
		PageResponseDTO<DataFuncResDTO> pageDataFunc= dataAuthManageSV.listDataFunc(dataFuncReqDto);
		if(pageDataFunc.getCount()!=1){
		...
		//3.遍历用户关联各角色下的功能规则集合
		for (Long staffRoleId : listStaffRole) {
			//找到角色权限关系
			AuthRole2Privilege authRole2Privilege = new AuthRole2Privilege();
			authRole2Privilege.setRoleId(staffRoleId);
			List<AuthRole2Privilege> listPrivilegeRoleRel = roleManageSV.listPrivilegeRoleRel(authRole2Privilege);
