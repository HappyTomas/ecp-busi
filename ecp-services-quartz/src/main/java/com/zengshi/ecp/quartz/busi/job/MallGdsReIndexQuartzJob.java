...
public class MallGdsReIndexQuartzJob extends AbstractCommonQuartzJob{
    
    private static final String MODULE = MallGdsReIndexQuartzJob.class.getName();
    ...
    protected void doJob(Map<String, String> extendParams) throws JobExecutionException {
        ...
        LogUtil.info(MODULE, "=============begin 人卫商城商品索引重建 job =============");
        try{
            SecConfigReqDTO secConfigReqDTO=new SecConfigReqDTO();
            secConfigReqDTO.setId(1l);
            this.secConfigPlanRSV.reFullImportIndex(secConfigReqDTO,false);

        } catch(Exception err){
            LogUtil.error(MODULE,"=============人卫商城商品索引重建 error =============",err);
        }
        LogUtil.info(MODULE,"=============end 人卫商城商品索引重建 job =============");
