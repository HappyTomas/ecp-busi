...
public class PromSendData2SolrQuartzJob extends AbstractCommonQuartzJob{
    
    private static final String MODULE = PromSendData2SolrQuartzJob.class.getName();
    ...
    protected void doJob(Map<String, String> extendParams) throws JobExecutionException {
        ...
        LogUtil.info(MODULE, "=============begin 促销通知solr job =============");
        try{
            //传入参数 站点编码 如果为空即空
            //传入参数 店铺编码 如果为空即为空
            ...
            LogUtil.error(MODULE,"=============促销通知solr BusinessException =============",err);
        }catch(Exception err){
            LogUtil.error(MODULE,"=============促销通知solr Exception =============",err);
        }
        LogUtil.info(MODULE,"=============end 促销通知solr job =============");
        
    }
    
    public  static void main(String[] args){
        PromSendData2SolrQuartzJob job=new PromSendData2SolrQuartzJob();
        try {
