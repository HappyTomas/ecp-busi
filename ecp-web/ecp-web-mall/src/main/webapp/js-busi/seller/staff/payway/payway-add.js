...
var up = {
		...
		add : function(){
			...
			var val = ebcForm.formParams($("#addForm"));
			$.eAjax({
	  			url : GLOBAL.WEBROOT + "/seller/payway/add",
	  			data : val,
	  			async : true,
				type : "post",
	  			dataType:'json',
...
$(function(){
	...
	$("#changeInput").click(function(){
		 if ($(this).attr('checked')) {
            $('#kNamec').hide();
            $(this).before('<input type="text" name="knameinput" maxlength="64" id="knameinput" value="" class="input-large required"> &nbsp;');
	...
	$("#changeInputCer").click(function(){
		if ($(this).attr('checked')) {
			$('#cNamec').hide();
			$(this).before('<input type="text" name="cnameinput" maxlength="64" id="cnameinput" value="" class="input-large required"> &nbsp;');
