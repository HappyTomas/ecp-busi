$(function() {
	...
	var pageInit = function() {
		...
		var init = function() {
			...
			promInfo.ifBlackList("#ifBlackList");
			//参与范围
			...
			var catgBlackTr;
			//分类删除按钮功能
			$("#catgBlackTable tr td a[ name='delGdsRow']").live('click',
					function(e) {
			...
			var catgTr;
			//分类删除按钮功能
			$("#catgTable tr td a[ name='delGdsRow']").live('click',
					function(e) {
						promInfo.delCatg(e);
					});
			...
			var gdsTr;
			//商品删除按钮功能
			$("#gdsTable tr td a[ name='delGdsRow']").live('click',
					function(e) {
						promInfo.delGds(e);
					});
			...
			var gdsBlackTr;
			//商品删除按钮功能
			$("#gdsBlackTable tr td a[ name='delGdsRow']").live('click',
					function(e) {
			...
			//checkbox黑名单分类事件
			$("#catgBlackTable thead tr input[ id='dt_row_all_check']").live(
					'click', function(e) {
						promInfo.selectCheckBoxCatgBlack(e);
					});
			//checkbox黑名单商品事件
			$("#gdsBlackTable thead tr input[ id='dt_row_all_check']").live(
					'click', function(e) {
			...
			$('#btnCatgOpenWindow').unbind('click').click(function() {
				var _sid = $('#siteId').val();
				var _shopId = $('#shopId').val();
				//	window.location.href=GLOBAL.WEBROOT+"/category/open/catgselect?catgType=1";
				promInfo.queryCatg(_sid,_shopId);
			});
			//黑名单分类选择 window
			$('#btnCatgBlackOpenWindow').click(function() {
				var _sid = $('#siteId').val();
				var _shopId = $('#shopId').val();
				promInfo.queryBlackCatg(_sid,_shopId);
			});
			//黑名单商品选择 window
			...
			$(".promCntBlur").live('blur',function(obj) {
								if ($('#doAction').val() == 'edit') {
									...
									if ($(obj.currentTarget).attr('oldvalue') != $(
											obj.currentTarget).val()) {
										var tr = $(obj.currentTarget).parent()
												.parent();
										var _skuId = $(tr).attr("id");//单品编码
										var _promCnt = $(obj.currentTarget)
												.val();
										//不是正整数
										if (!ebcCheck.isInt(_promCnt)) {
											eDialog.alert('促销数量不是正整数，请检查',
															null);
											return;
										}
										promInfo.updateGdsPromCntDb(
												$(obj.currentTarget), _skuId,
												_promCnt);
									}
								}
							});
	...
	var promInfo = {
		...
		promInfoShow : function(_doAction) {
			//触发调用 查询商品事件
			if (_doAction == "view" || _doAction == "edit"
					|| _doAction == "copy") {
				...
				if ($('#ifBlackList').prop('checked') == true) {
					...
					//黑名单 可选
					$('#btnGdsBlackOpenWindow').attr("disabled",false);
					//黑名单 可选
					$('#btnCatgBlackOpenWindow').attr("disabled",false);
					$('#btnGdsBlackOpenWindow').addClass("sbtn-blue");
					$('#btnCatgBlackOpenWindow').addClass("sbtn-blue");
				} else {
					//黑名单 disabled
					$('#btnGdsBlackOpenWindow').attr("disabled",true);
					$('#btnCatgBlackOpenWindow').attr("disabled",true);
					$('#btnGdsBlackOpenWindow').removeClass("sbtn-blue");
					$('#btnCatgBlackOpenWindow').removeClass("sbtn-blue");
				}
		...
		queryGroup : function(_siteId) {
			bDialog.open({
				...
				width : 800,
				height : 570,
				url : GLOBAL.WEBROOT + "/seller/querygroup?siteId=" + _siteId,
				callback : function(data) {
					if (data && data.results && data.results[0]) {
						$('#groupId').val(data.results[0].id);
						$('#groupName').val(data.results[0].pormTheme);
					}
		...
		queryBlackGds : function(_shopId, _siteId) {
			bDialog.open({
				...
				width : 800,
				height : 560,
				url : GLOBAL.WEBROOT + "/seller/gdsprom/gdsblackselect?shopId="
						+ _shopId + "&siteId=" + _siteId,
				callback : function(data) {
					//确定 按钮_if_query==1
					if (data && data.results
							&& data.results[0]._if_query == '1') {
		...
		//黑名单分类
		queryBlackCatg : function(_siteId,_shopId) {
			bDialog.open({
				title : '分类选择',
				width : 350,
				height : 550,
		...
		//查询分类
		queryCatg : function(_siteId,_shopId) {
		...
		queryGds : function(_shopId, _siteId) {
			bDialog.open({
				...
				width : 800,
				height : 560,
				url : GLOBAL.WEBROOT + "/seller/gdsprom/gdsselect?shopId=" + _shopId
						+ "&siteId=" + _siteId,
				callback : function(data) {
					//确定 按钮_if_query==1
					if (data && data.results
		...
		selectCheckBoxGdsBlack : function(e) {
			...
			if (e.currentTarget.checked) {
				$("#gdsBlackTable tbody tr td input[ name='checkboxgds']")
						.prop('checked', true);
			} else {
				//全部取消
				$("#gdsBlackTable tbody tr td input[ name='checkboxgds']")
		...
		selectCheckBoxCatgBlack : function(e) {
			//选中 表示全选
			if (e.currentTarget.checked) {
				$("#catgBlackTable tbody tr td input[ name='checkboxcatg']")
						.prop('checked', true);
			} else {
				//全部取消
				$("#catgBlackTable tbody tr td input[ name='checkboxcatg']")
		...
		selectCheckBoxGds : function(e) {
			//选中 表示全选
			if (e.currentTarget.checked) {
				$("#gdsTable tbody tr td input[ othername='checkboxgds']").prop(
						'checked', true);
			} else {
				//全部取消
				$("#gdsTable tbody tr td input[ othername='checkboxgds']").prop(
		...
		selectCheckBoxCatg : function(e) {
			//选中 表示全选
			if (e.currentTarget.checked) {
				$("#catgTable tbody tr td input[ name='checkboxcatg']").prop(
						'checked', true);
			} else {
				//全部取消
				$("#catgTable tbody tr td input[ name='checkboxcatg']").prop(
		...
		initBlackCatgList : function(_catgIds) {
			...
			//ajax请求
			$.eAjax({
				url : GLOBAL.WEBROOT + "/seller/gdsprom/catgblackList",
				data : [ {
					name : 'promId',
					value : _promId
				}, {
		...
		initBlackGdsList : function(_skuIdsBlack) {
			...
			//ajax请求
			$.eAjax({
				url : GLOBAL.WEBROOT + "/seller/gdsprom/gdsblackList?promId="
						+ _promId + "&skuIds=" + _skuIdsBlack,
				"dataType" : "text",
				success : function(returnInfo) {
		...
		initCatgList : function(_catgIds) {
			...
			//ajax请求
			$.eAjax({
				url : GLOBAL.WEBROOT + "/seller/gdsprom/catgList",
				data : [ {
					name : 'promId',
					value : _promId
				}, {
		...
		initGdsList : function(_skuIds, ifmerge) {
			...
			//ajax请求
			$.eAjax({
				url : GLOBAL.WEBROOT + "/seller/gdsprom/gdsList",
				data : [ {
					name : 'promId',
					value : _promId
				}, {
		...
		joinRangeClick : function(_obj) {
			if ($(_obj).val() == '1') {
				...
				//黑名单 不可用
				$('#btnGdsBlackOpenWindow').addClass("sbtn-disabled");
				$('#btnCatgBlackOpenWindow').addClass("sbtn-disabled");
				$('#btnGdsBlackOpenWindow').removeClass("sbtn-blue");
				$('#btnCatgBlackOpenWindow').removeClass("sbtn-blue");
		...
		ifBlackList : function(_obj) {
			if ($(_obj).prop('checked') == true) {
				...
				$('#btnGdsBlackOpenWindow').addClass("sbtn-blue");
				$('#btnCatgBlackOpenWindow').addClass("sbtn-blue");
				$('#btnGdsBlackOpenWindow').removeClass("sbtn-disabled");
				$('#btnCatgBlackOpenWindow').removeClass("sbtn-disabled");
				//黑名单 可选
				$('#btnGdsBlackOpenWindow').attr("disabled",false);
				//黑名单 可选
				$('#btnCatgBlackOpenWindow').attr("disabled",false);
		...
		importData : function(id, action) {
			bDialog.open({
				...
				width : 900,
				height : 560,
				url : GLOBAL.WEBROOT + "/seller/gdsprom/importGds?promId=" + id,
				params : {},
				onHidden : function() {
					//需要刷新 商品列表
					promInfo.initGdsList(null, null);
		...
		manyGds : function(id, action) {
			bDialog.open({
				...
				width : 900,
				height : 500,
				url : GLOBAL.WEBROOT + "/seller/gdsprom/manyGds?promId=" + id
						+ "&doAction=" + action + "&promTypeCode="+ $('#promTypeCode').val(),
				params : {
					'promId' : id,
					'doAction' : action
				},
		...
		deleteGdsDb : function(_promId, skuIds) {
			$.eAjax({
				url : GLOBAL.WEBROOT + "/seller/gdsprom/batchDel",
				data : [ {
					name : 'promId',
					value : _promId
				}, {
		...
		updateGdsPromCntDb : function($obj, skuIds, promCnt) {
			var _promId = $('#promId').val();
			$.eAjax({
				url : GLOBAL.WEBROOT + "/seller/gdsprom/updateCnt",
				data : [ {
					name : 'promId',
					value : _promId
				}, {
		...
		insertGdsList : function(_skuIds) {
			...
			//ajax请求
			$.eAjax({
				url : GLOBAL.WEBROOT + "/seller/gdsprom/insertGds",
				data : [ {
					name : 'promId',
					value : _promId
				}, {
