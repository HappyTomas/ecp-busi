$(function() {
			...
			$("#btnMatchGdsSelect").live('click',function(e) {
								...
								bDialog
										.open({
											...
											height : 550,
											url : GLOBAL.WEBROOT
													+ "/seller/gdsprom/gdsselect?shopId="+ $('#shopId').val()
													+ "&siteId=" + _siteId,
	...
	var discountRuleFun = {
			...
			if (_l.length <= 1) {
				...
				for (var i = 0; i < _l.length; i++) {
					...
					// 实体需要验证库存
					 if(_isNeedStock=="true"){
						if (!this.isNum(_promCnt)) {
							eDialog.alert('优惠规则，商品编码-单品编码：' + _l[i].id
									+ " 对应的<span style='color:red'>搭配总量</span>，请输入有效的数字  " + _promCnt + " ！",
									null);
							_reData.value = false;// 返回默认 失败
							return _reData;
						}
					...
					if (!this.checkPriceNum(_price)) {
						eDialog.alert('优惠规则，商品编码-单品编码：' + _l[i].id
								+ " 对应的<span style='color:red'>单价</span>，请输入有效的数字(最多保留2位小数)" + _price + " ！",
								null);
						_reData.value = false;// 返回默认 失败
						return _reData;
					}
					//验证排序
					_sort = $('#sort-' + _l[i].id).val();
					...
					data.push({
						'name' : 'matchMap[' + _l[i].id + ']',
						'value' : _promCnt + "-" + _price+"-"+_sort
					});
				}
			}
			_reData.value = true;
		...
		initList : function(_skuIds) {
			...
			// ajax请求
			$.eAjax({
				url : GLOBAL.WEBROOT + "/seller/matchprom/matchList",
				data : [ {
					name : 'promId',
					value : _promId
				}, {
