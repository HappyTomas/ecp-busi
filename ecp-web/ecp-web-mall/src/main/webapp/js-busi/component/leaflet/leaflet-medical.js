...
(function(factory) {
	...
	$image_medical = {
		...
		"doData" : function(el, datas,opts) {
			if(datas && datas.respList && datas.respList.length > 0){
				$image_medical.doAdList($(el),datas.respList,opts);
			}else{//无数据
				$(el).empty();
				$(el).append("<div class ='pro-empty'>亲，暂未提供下载！</div>");
			}
		},
		"control":function(el,opts){
			...
			$AdData.getData({
				...
				"callback":function(ads){
					$image_medical.doData(el,ads,opts);
				}
			});
		},
		"doAdList":function(_obj,dataList,opts){
			...
			for(var i = 0;i < dataList.length;i++){//循环广告
				if(i==0){
					str += "<div class=\"ele-ewm "+firstMT +" "+ borderBox +"\">";
				}
				else{
					str += "<div class=\"ele-ewm mt10 "+borderBox+"\">";
				}
	...
	$.fn.image_medical = function() {
		return this.each(function() {
			...
			$image_medical.control(this,opts);
