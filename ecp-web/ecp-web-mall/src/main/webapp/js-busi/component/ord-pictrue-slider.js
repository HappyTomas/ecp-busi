...
(function(factory) {
	...
	if (typeof define === "function" && define.amd) {
		...
		define(['jquery-icheck',
		        'jquery-jqzoom','jquery-powerSwitch','ecp-component/ord-pictrue-data' ], function(powerSwitch,icheck,jqzoom,gdsPictrue) {
			factory(jQuery);
		...
		"getData" : function(el, datas) {
			...
			if(datas.result.length>=1){
				$.each(datas.result, function(i, n) {
					if(i==0){
						htmlContext += "<div class='pre-img'>"+
			                "<a href='"+n.bigUrl+"' rel='gal1' class='jqzoom'>"+
			                    "<img class='pcimg' height='"+pictrueMoreHeight/2+"' width='"+pictrueMoreWidth/2+"'  src='"+n.middleUrl+"' alt='' style='margin: 0px;'/>"+
			                "</a>"+
			            "</div>"+
						"<div class='pre-list'>"+
		                "<div class='slide jcarousel'>"+
		                    "<div id='zoomimgn' class='slide-trigger'>"+
		                        "<a href='javascript:' class='' data-rel='zimg1'></a>"+
		                        "<a href='javascript:' class='' data-rel='zimg1'></a>"+
		                    "</div>"+
		                    "<div class='slide-box' style='height:80px;'>"+
					...
					htmlContext +=    "<img style='margin: 0px;' m='"+n.middleUrl+"' b='"+n.bigUrl+"' width='"+pictrueHeight+"' height='"+pictrueWidth+"' src='"+n.url+"' alt='Image 1'>"+
	                                "</a>"+
	                            "</li>";
				});
				htmlContext +="</ul>"+
