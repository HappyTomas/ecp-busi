...
(function(factory) {
		...
		define([ 'scrollTo','onePageNav'], function(scrollTo,onePageNav) {
			...
			return {
				componentInit : function() {
					$(".homepage-comment").homepage_elevator();
				}
			};
	...
	var $homepage_comment = {
		...
		"doData" : function($container, data) {
			...
			$.each(data, function(i, item) {
                ...
                ' <div class="hot-pl-com" data-content="'+(item.detail?item.detail:"")+'">'+
                ' <span>'+(item.staffName?item.staffName:"")+':</span>'+
                ' <a href='+GLOBAL.WEBROOT+item.gdsDetailUrl+'><span class="hot-detail">'+(item.detail?item.detail:"")+'</span></a>'+
                ' </div>';
	            ...
	            if(intScore){
	            	 ...
	            	 for(var k=0;k<intScore;k++){
	            		 scoreStr+='<img src="'+GLOBAL.WEBROOT+'/images/x.png" alt=""/>';
	 	              }
	            	 scoreStr+='</p>';
	            }
	            scoreStr+='</li>'
			...
			if(containHeight > divHeight){
				startmarquee($("#hotScroll"),50,50,0);
			}
		    $('.hot-pl .hot-pl-com').popover({trigger: 'hover', placement: 'bottom'})
		},
		"control":function(el,opts){
			var $container = $(".homepage-comment-cont", el);
			$container.append('<div class="loading"></div>');
			$.eAjax({
				url : $webroot + '/homepage/hotComment',
				data : {
					"status" : opts.status,
					"size" : opts.size,
				},
				...
				success : function(data, textStatus) {
					if (data && data.length > 0) {
						$homepage_comment.doData($container,data);
					} else {
						$container.empty();
	...
	$.fn.homepage_elevator = function() {
		return this.each(function() {
			...
			$homepage_comment.control(this,opts);
