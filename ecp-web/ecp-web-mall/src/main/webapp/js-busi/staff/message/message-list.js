$(function(){
    var pInit = function(){
    	var init = function(){
	    	...
	    	//载入"未读消息"页面
	    	$('#dataBody').load(GLOBAL.WEBROOT + '/msginsite/unread?v='+Math.random());
	    	...
	    	$('#all').click(function(){
	    		...
	    		$('#read').css("color","");
	    		//加载指定页面
	    		$('#dataBody').load(GLOBAL.WEBROOT + '/msginsite/msglist?v='+Math.random());
	    	});
	    	//绑定查询按钮事件
	    	$('#unread').unbind('click');
	...
	if(_idsArr && _idsArr.length>=1){
		...
		eDialog.confirm("您确认要批量删除吗？", {
			buttons : [{
				...
				callback : function(){
					$.eAjax({
						url : GLOBAL.WEBROOT + "/msginsite/batchdel",
						data : {'ids':_data},
						datatype:'json',
	...
	if(_idsArr && _idsArr.length>=1){
		...
		eDialog.confirm("您确认要标记为已读吗？", {
			buttons : [{
				...
				callback : function(){
					$.eAjax({
						url : GLOBAL.WEBROOT + "/msginsite/markread",
						data : {'ids':_data},
						datatype:'json',
						success : function(returnInfo) {
