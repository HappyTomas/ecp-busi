    ...
    var pageInit = function(){
        ...
        var init = function(){
            ...
            var detailOrd = function(_this){
        		...
        		window.open(GLOBAL.WEBROOT + "/ord/point/detail/"+orderId);
        		//$('#detailForm').submit();
            }
        	...
        	$('#teamTab li a').click(function(){
        		...
        		window.location.href = GLOBAL.WEBROOT + '/order/point/'+controllConfig[id];
        	});
        	...
        	$('#pageControlbar').bPage({
        	    url : GLOBAL.WEBROOT + '/order/point/'+controllConfig[constant.status],
        	    asyncLoad : false,
        	    params : function(){
...
/*        		var shopId = $(this).parents('.order-tb').find('input[name="shopId"]').val();
        		var isStatus = true;
        		var url = GLOBAL.WEBROOT + "/point/pay/check"; 
        		$.eAjax({
        			url:url,
        			...
        			success:function(returnInfo){
        				if(returnInfo&&returnInfo.flag==false){  
        					...
        					eDialog.alert(returnInfo.msg,function(){
        						window.location.href = GLOBAL.WEBROOT + "/order/point/pay";
        					}); 
        				}  
        			}
        		});   
        		if(isStatus == true){
        			window.open(GLOBAL.WEBROOT + "/point/pay/queryWay?orderId="+orderId+"&shopId="+shopId);
        		} */
				payCheck(orderId);
        	});
        		...
        		eDialog.confirm("取消订单" , {
        		    buttons : [{
        		        ...
        		        'callback' : function(){
        		        	...
        		        	var url = GLOBAL.WEBROOT + "/ord/point/myorder/cancel";
        		        	$.eAjax({
        	        			url:url,
        	        			data:[{name:"orderId",value:orderId},{name:"oper",value:"04"}],
        	        			method:'post',
			...
			$('._update').live('click',function(){
		        ...
		        bDialog.open({
		            ...
		            url : GLOBAL.WEBROOT+'/ord/point/buyeraddrupdate?orderId='+orderId,
        	...
        	$('._confirm').click(function(){
        		...
        		eDialog.confirm("收货确认" , {
        		    buttons : [{
        		        ...
        		        'callback' : function(){
        		        	var orderId = $(_this).closest('.order-tb').find("input[name='orderId']").val();
        		        	var url = GLOBAL.WEBROOT + "/order/point/confirmord";
        		        	$.eAjax({
        	        			url:url,
        	        			...
        	        			success:function(returnInfo){
        	        				if(returnInfo&&returnInfo.resultFlag=='true'){
        	        					window.location.href = GLOBAL.WEBROOT + "/order/point/recept";
        	        				}else{
        	        					eDialog.alert(returnInfo.resultMsg,function(){
        	        						window.location.href = GLOBAL.WEBROOT + "/order/point/recept";
        	        					});
        	...
        	$('._returnApply').click(function(){
	        	...
	        	if(isStatus == true){
	        		window.open(GLOBAL.WEBROOT + "/order/point/return/returnApply/"+orderId);
        		} 
        	});
        	...
        	$('._returnChild').click(function(){
	        	...
	        	if(isStatus == true){
	        		window.open(GLOBAL.WEBROOT + "/order/point/return/returnChild/"+orderId);
        		}
        	}); 
        };
