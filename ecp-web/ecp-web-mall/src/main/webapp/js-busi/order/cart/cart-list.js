	...
	var pInit = function(){
		var init = function(){
			...
			var checkItemCount = function(){
				var itemCount = 0;
				//普通商品个数统计
				$('#cartList :checkbox:checked').not('.groupCheck').each(function(i,row){
			...
			var shopCount = function(){
				...
				$('.car-item').each(function(i,row){
					...
					$('span.itemPriceCount',$(_this)).each(function(i,itemPrice){
						...
						//选中统计店铺减免金额
						if(total>0)shopPromDiscountMoney = Number($('.shopPromDiscountMoney',$(_this)).val());
					});
					//店铺减免金额前的价格小计
...
//				$('#cartAmountConut').text(ebcUtils.thousandSeparator(ebcUtils.numFormat(orderTotal-discountTotal>0?orderTotal-discountTotal:0)));
				$('#cartAmountConut').text(ebcUtils.thousandSeparator(ebcUtils.numFormat(orderBaseTotal-shopDiscountTotals>0?orderBaseTotal-shopDiscountTotals:0)));
				//计算订单总优惠金额
			...
			var calcGroupItem = function(obj,srcNum,modify){
				...
				//排除组合商品系列
				if($(box).hasClass('item-body-tit')) return;
				var discountPriceMoney = 0;
				var itemPromCaclPrice = Number($('.unitBasePrice',$(box)).text());
				var itemPromFinalPrice = Number($('.groupItemPrice',$(box)).text());

				var number = $('.groupAmounts',$(box)).val();
			...
			var calcItem = function(obj,srcNum,modify){
				...
				var setPriceCount = function(finalPrice,caclPrice,number){
					...
					if(itemPromCaclPrice == itemPromFinalPrice)
			...
			$('.p-reduce').click(function(e){
				e.stopPropagation();
				hideListItem();
				var itemPromId = $('#itemTempPromId',$(this).closest('.item-body')).val();
				$(':input:radio',$(this)).prop('checked',false).each(function(){
					if(itemPromId == $(this).val()) $(this).prop('checked',true);
				});
			...
			$('.shop-reduce').click(function(e){
				e.stopPropagation();
				hideListItem();
				var shopPromId = $('#shopTempPromId',$(this).closest('.car-item')).val();
				$(':input:radio',$(this)).prop('checked',false).each(function(){
					if(shopPromId == $(this).val()) $(this).prop('checked',true);
				});
				$(this).find('.body').show();
			...
			$('.shopName').click(function(){
				...
				// url = GLOBAL.WEBROOT+"shopIndex/"+shopId;
				var box = $(this).closest('.shop');
				var shopId = $('.shopId',$(box)).val();
				url = $webroot+"shopIndex/"+shopId;
				window.open(url);
			});
