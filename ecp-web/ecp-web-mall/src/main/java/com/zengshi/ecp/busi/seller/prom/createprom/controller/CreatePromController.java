...
import com.zengshi.ecp.busi.seller.prom.PromUtil;
import com.zengshi.ecp.busi.seller.prom.createprom.vo.PromInfoVO;
import com.zengshi.ecp.busi.seller.prom.createprom.vo.PromRuleVO;
import com.zengshi.ecp.busi.seller.prom.createprom.vo.PromShipAddress;
import com.zengshi.ecp.busi.seller.prom.createprom.vo.PromTypeVO;
import com.zengshi.ecp.busi.seller.prom.createprom.vo.PromVO;
import com.zengshi.ecp.busi.seller.prom.createprom.vo.QueryPromType4ShopVO;
 ...
 */
@Controller
@RequestMapping("/seller/createprom")
public class CreatePromController extends EcpBaseController {
     ...
     */
    @RequestMapping()
    public String init(Model model, QueryShopVO queryShopVO ){
    	  // 组织参数
/*        ShopSelectReqDTO  conds=new ShopSelectReqDTO();
        ...
        PageResponseDTO<ShopInfoResDTO> shopPage=shopInfoRSV.listShopInfoByCond(conds);
        
        model.addAttribute("shopPage",shopPage );*/
        
        return "/seller/prom/createprom/query-shop-grid";
    }
    /**
     * 店铺查询列表分页
     * @param model
    ...
    public String shopList(Model model, QueryShopVO queryShopVO) throws Exception {
        ...
        PageResponseDTO<ShopInfoResDTO> shopPage=new PageResponseDTO<ShopInfoResDTO>();
        ...
        shopPage=shopInfoRSV.listShopInfoByCond(conds);
        model.addAttribute("shopPage", shopPage);

      return "/seller/prom/createprom/shop/shop-list";
    }
	/**
	 * 促销tab页面
	 * /{shopId}/{promTypeCode}
	 * @return   @PathVariable("shopId") String shopId, @PathVariable("promTypeCode") String promTypeCode
	 * @author lisp
	 */
    @RequestMapping(value="/newprom")
	public String promTab(PromTypeVO PromTypevo,
			Model model) {
		// 获得促销类型对象
		PromTypeDTO promTypeDTO = new PromTypeDTO();
		
		String promTypeCode = PromTypevo.getPromTypeCode();
		...
		model.addAttribute("promType", p);
		 ...
		 model.addAttribute("shopId", PromTypevo.getShopId());
		...
		vo.setPromTypeCode(promTypeCode);
		 vo.setShopId(new Long(PromTypevo.getShopId()));
		...
		return "/seller/prom/createprom/prom-tab";
	}
		  ...
		  //返回页面vo定义
	    	...
	    	if (CheckPageNull.checkPageNull(page)) {
				  for(PromType4ShopResponseDTO dto:page.getResult()){
					        ...
					        PromTypeDTO promTypeDTO = new PromTypeDTO();
					        promTypeDTO.setPromTypeCode( dto.getPromTypeCode());
					        PromTypeResponseDTO p = promQueryRSV.queryPromType(promTypeDTO);
					        ObjectCopyUtil.copyObjValue(p, promTypeVO, "", false);
					        promTypeVO.setShopId(dto.getShopId());
					        promTypeVOList.add(promTypeVO);
				  }
			}
		return "/seller/prom/createprom/prom-show-form";
	}
	...
	private void initPageData(Model model, PromVO promVO) {
		...
		// 客户组
		 ...
		 model.addAttribute("custGrouplList", custGrouplList);
	    // model.addAttribute("custLevelList",queryCustGroupData(promVO.getPromInfoVO().getShopId(), StaffConstants.ShopStaffGroup.STATUS_ACTIVE));
		...
		// 是否显示 展示开始时间 展示截止时间
		model.addAttribute("ifShowShowTime", PromUtil.matchSysCfgValue(PromConst.PromSys.PROM_TYPE_HIDE_SHOW_TIME,
				promVO.getPromInfoVO().getPromTypeCode()));

		// 是否可操作 分类、商品、单品选择操作
		model.addAttribute("ifShowSelect", PromUtil.matchSysCfgValue(PromConst.PromSys.PROM_TYPE_SYS_CFG_KEY,
				promVO.getPromInfoVO().getPromTypeCode()));
		// 是否可操作 促销规则tab页面
		model.addAttribute("ifShowPromRuleTab",
				PromUtil.matchSysCfgValue(PromConst.PROM_RULE_IF_HIDE, promVO.getPromInfoVO().getPromTypeCode()));
		// 加载邮费 区域信息
		...
		if (PromConst.PROM_TYPE_CODE_1000000016.equals(promVO.getPromInfoVO().getPromTypeCode())) {
			List<BaseAreaAdminRespDTO> provinceListAll = BaseAreaAdminUtil
