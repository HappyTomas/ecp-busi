...
import com.zengshi.ecp.busi.seller.goods.vo.mediacategory.MediaCategoryRespVO;
import com.zengshi.ecp.busi.seller.goods.vo.mediacategory.MediaCategoryTreeVO;
import com.zengshi.ecp.busi.seller.goods.vo.mediacategory.MediaCategoryVO;
 ...
 */
@Controller
@RequestMapping("/seller/goods/mediacatg")
public class MediaCategoryController extends GdsBaseController{
	
	private static final String BASE_URL = "/seller/goods/mediacatg/";
    ...
    public String mainPage(Model model,MediaCategoryVO reqVO)throws Exception{
    	...
    	return BASE_URL+"mediacatg-main";
    }
...
//    @ResponseBody
    ...
    public String getNodes(Model model,MediaCategoryVO reqVO)throws Exception{
    	...
    	if(StringUtil.isBlank(reqVO.getId())){
        	...
        	if(CollectionUtils.isNotEmpty(linkShops)){
				for (Iterator<ShopInfoResDTO> iterator = linkShops.iterator(); iterator
						.hasNext();) {
					ShopInfoResDTO shop =  iterator
							.next();
					MediaCategoryTreeVO item = buildShopNodeItem(shop, null);
					tree.add(item);
				}
        	}
    	}else{
    ...
    public String categorySearch(Model model,MediaCategoryVO reqVO)throws Exception{
    	...
    	if(CollectionUtils.isNotEmpty(nodes)){
    		...
    		if(CollectionUtils.isNotEmpty(linkShops)){
        		for (Iterator<ShopInfoResDTO> iterator = linkShops.iterator(); iterator
						.hasNext();) {
        			ShopInfoResDTO sResDTO = iterator.next();
        			if(GdsUtils.isEqualsValid(sResDTO.getShopStatus())){
        			    shopMap.put(sResDTO.getId(), sResDTO);
        			}
				}
        	}
  ...
  /*  public String repairGds2Catg(MediaCategoryVO reqVO){
    ...
    */
    ...
    private MediaCategoryTreeVO buildShopNodeItem(ShopInfoResDTO dto,Boolean isOpen) {
		...
		vo.setIsRoot(true);
		vo.setClick(false);
		vo.setShopId(dto.getId().toString());
		vo.setOpen(isOpen);
		return vo;
	}
