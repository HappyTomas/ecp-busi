...
import com.zengshi.ecp.busi.seller.coup.coupinfo.vo.CoupVO;
import com.zengshi.ecp.busi.seller.coup.couptype.vo.CouponTypeVO;
...
@Controller
@RequestMapping(value = "/seller/coup")
public class CoupController extends EcpBaseController {
    ...
    @ResponseBody
    public CoupVO submit(@ModelAttribute("coupVO")  @Valid  CoupVO coupVO) {
    ...
    public String coupTab(@PathVariable("coupTypeId") String coupTypeId,Model model){
        ...
        if(sellerResDTO.getShoplist().size()>0){
    		vo.setShopId(sellerResDTO.getShoplist().get(0).getId());
    	}
        //有效日期类型
    ...
    public String goToCode(@PathVariable("coupCode") String coupCode,  Model model) {
        ...
        model.addAttribute("coupVO", coupVO);
        return "/seller/coupon/coup-code";
    }
}
