...
//            resultMap.put("vfsUrl", ImageUtil.getStaticDocUrl(vfsId, "doc"));
    ...
    public void uploadImage(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if (file == null) {
                ...
                out.print(net.sf.json.JSONObject.fromObject(resultMap).toString());
                return;
            }
            String fileName = file.getOriginalFilename();
            String extensionName = "." + getExtensionName(fileName);
            ...
            byte[] datas = inputStream2Bytes(file.getInputStream());
            if (datas.length > 1 * 100 * 1024) {
            	resultMap.put("success", EcpBaseResponseVO.RESULT_FLAG_FAILURE);
            	resultMap.put("message", "附件上传失败，上传的文件必须小于100K!");
                LogUtil.error(MODULE, "附件上传失败，上传的文件必须小于100K!");
                return ;
            }
    ...
    public boolean isShopValid(Long shopId){
        ...
        if(CollectionUtils.isNotEmpty(sellerResDTO.getShoplist())){
            for(ShopInfoResDTO shopInfoResDTO:sellerResDTO.getShoplist()){
                if(shopInfoResDTO.getId()==shopId){
                    flag=true;
                    break;
                }
            }
