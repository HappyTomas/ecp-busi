...
import com.zengshi.ecp.busi.seller.order.vo.RAuditTradeCheckReqVO;
import com.zengshi.ecp.busi.seller.order.vo.ROfflineQueryReqVO;
import com.zengshi.ecp.busi.seller.order.vo.ROrderBackReqVO;
...
@Controller
@RequestMapping(value = "/seller/order/bankpay")
public class BankPayController extends EcpBaseController{

    private static final String MODULE = BankPayController.class.getName();
    ...
    public String init(Model model) {  
        ...
        return "/seller/order/pay/bankpay-main";
    }
    ...
    public String yhPayList(Model model, RAuditTradeCheckReqVO vo) throws Exception {
        if(StringUtil.isNotBlank(vo.getStaffCode())) {
            ...
            if (custInfoResDTO==null) {
				return "/seller/order/pay/tab/yhPayTab";
			}
        }
        ...
        dto.setEndTime(new Timestamp(DateUtils.addDays(vo.getEndTime(), 1).getTime()));
        ...
        model.addAttribute("bankPayListPage", ParamsTool.ordDetailSiteUrl(respVO));
        return "/seller/order/pay/tab/yhPayTab";
        }
    ...
    public String yhBackList(Model model, RAuditTradeCheckReqVO vo) throws Exception { 
        if(StringUtil.isNotBlank(vo.getStaffCode())) {
            CustInfoReqDTO CustInfoReqDTO = new CustInfoReqDTO();
            CustInfoReqDTO.setStaffCode(vo.getStaffCode());
            CustInfoResDTO custInfoResDTO = staffUnionRSV.findCustInfo(CustInfoReqDTO);
            if(custInfoResDTO!=null) vo.setStaffId(custInfoResDTO.getId());
            if (custInfoResDTO==null) {
				return "/seller/order/pay/tab/yhBackTab";
			}
        }
        RAuditTradeCheckRequest dto = new RAuditTradeCheckRequest();
        dto = vo.toBaseInfo(RAuditTradeCheckRequest.class);
    ...
    public String ptPayList(Model model, RQueryOrderReqVO vo) throws Exception {

        RQueryOrderRequest dto = new RQueryOrderRequest();
        dto = vo.toBaseInfo(RQueryOrderRequest.class);
        ObjectCopyUtil.copyObjValue(vo, dto, "", false); 
        //dto.setEndDate(new Timestamp(DateUtils.addDays(vo.getEndDate(), 1).getTime()));
        LogUtil.debug(MODULE, vo.toString()); 
        PageResponseDTO<ROrdMainResponse> t = ordMainRSV.querynotInAuditTradeCheckOrders(dto);
        EcpBasePageRespVO<Map> respVO = EcpBasePageRespVO.buildByPageResponseDTO(t);
        model.addAttribute("bankPayListPage", ParamsTool.ordDetailSiteUrl(respVO));
        return "/seller/order/pay/tab/ptPayTab";         
    }
    ...
    public String ptBackList(Model model, ROrderBackReqVO vo) throws Exception {
...
/*        dto.setEndDate(new Timestamp(DateUtils.addDays(vo.getEndDate(), 1).getTime()));*/
        ...
        EcpBasePageRespVO<Map> respVO = EcpBasePageRespVO.buildByPageResponseDTO(t);
        model.addAttribute("bankPayListPage", ParamsTool.ordDetailSiteUrl(respVO));
        return "/seller/order/pay/tab/ptBackTab";
     } 
    ...
    public void exportYhPayExcel(HttpServletResponse response,HttpServletRequest request, @ModelAttribute
    ...
    public void exportYhBackExcel(HttpServletResponse response,HttpServletRequest request, @ModelAttribute RAuditTradeCheckReqVO vo) {
    ...
    public void exportPtPayExcel(HttpServletResponse response,HttpServletRequest request, @ModelAttribute RQueryOrderReqVO vo) {
    ...
    public void exportPtBackExcel(HttpServletResponse response,HttpServletRequest request, @ModelAttribute ROrderBackReqVO vo) {
