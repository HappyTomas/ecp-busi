...
public class WeChatNotifyController {
    ...
    public void weChatNotify(HttpServletRequest request, HttpServletResponse response) throws Exception {
        logger.info("微信支付异步回调处理-----start1------!");
        response.setContentType("text/html");
        request.setCharacterEncoding("utf-8");
        response.setCharacterEncoding("utf-8");
        PrintWriter out = response.getWriter();
        logger.info("微信支付异步回调处理-----start2------!");
        String payWay = "9006";
        ...
        tAipSvLogWithBLOBs.setAppKey("WeChatNotifyController9006");
        tAipSvLogWithBLOBs.setServiceName("WeChatNotifyController");
        tAipSvLogWithBLOBs.setRequest(payInfo.toString());
        tAipSvLogWithBLOBs.setRequestTime(requestTime);
        Map<String, String> returnMap = new HashMap<String, String>();
        try {
        	logger.info("微信支付异步回调处理-----start3------2016.10.29遇到邪门bug我不信了得测试一把!");
/*            OrderPayStatusVO orderPayStatusVO = new OrderPayStatusVO();
            orderPayStatusVO = paymentRSV.parsePayStatus(payWay, payInfo);*/
