...
public class AlipayRefundNotifyController {
	private final static Logger logger = LoggerFactory.getLogger(AlipayRefundNotifyController.class);
	...
	@RequestMapping(value = "/alipayrefundnotify", method = { RequestMethod.POST,RequestMethod.GET })
	public void alipayNotify(HttpServletRequest request,HttpServletResponse response) throws Exception {
		response.setContentType("text/html");
		request.setCharacterEncoding("utf-8");
		response.setCharacterEncoding("utf-8");
		PrintWriter out = response.getWriter();
		logger.info("Begin to deal AlipayRefundNotifyController data!");
		String payWay = "9003";
		...
		tAipSvLogWithBLOBs.setAppKey("AlipayRefundNotifyController9003");
		tAipSvLogWithBLOBs.setServiceName("AlipayRefundNotifyController");
		...
		try {
			...
			if (returnMap != null && returnMap.get(flag) != null&& returnMap.get(message) != null) {
				...
				if (returnMap.get(flag).equalsIgnoreCase(success_status)) {
					...
					logger.info("AlipayRefundNotifyController处理成功！");
					String refundReturnUrl = returnMap.get(REFUNDRETURNURL);
                    if(StringUtil.isNotBlank(refundReturnUrl)){
                        logger.info("跳转至退货退款页面");
                        response.sendRedirect(refundReturnUrl);
                    }
				} else {
					out.write(returnMap.get(message));
					logger.error("AlipayRefundNotifyController处理失败！失败信息："+ returnMap.get(message));
				}
			} else {
		...
		logger.info("AlipayRefundNotifyController处理结束！");
	}
