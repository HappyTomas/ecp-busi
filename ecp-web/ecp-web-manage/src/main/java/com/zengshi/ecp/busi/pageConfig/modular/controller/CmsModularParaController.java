...
import com.zengshi.ecp.busi.pageConfig.prop.vo.CmsModularPropVO;
 ...
 */
@Controller
@RequestMapping(value="/modularPara")
public class CmsModularParaController extends EcpBaseController {
	private String MODUAL = CmsModularParaController.class.getName();
    private String URL = "/pageConfig/modular/modularPara";//返回页面的基本路径
    @Resource
     ...
     * @since JDK 1.6
     */
    @RequestMapping("/selectProp")
    public String selectProp(Model model,Long modularId) throws Exception{
    	model.addAttribute("modularId", modularId);
    	return "/pageConfig/modular/modularProp/modularProp-select";
    }
    /**
     * 
     * gridlist:(页面初始化). <br/> 
    ...
    @ResponseBody
    public Model selectlist(Model model,CmsModularParaPropVO cmsModularParaPropVO) throws Exception{
     ...
     * @since JDK 1.6
     */
    @RequestMapping("/saveSelectProp")
    @ResponseBody
    public EcpBaseResponseVO saveSelectProp(Model model,CmsModularPropVO cmsModularPropVO) throws Exception{
    	EcpBaseResponseVO ecpBaseResponseVO = new EcpBaseResponseVO();
        try {
        	CmsModularPropReqDTO cmsModularPropReqDTO=new CmsModularPropReqDTO();
        	cmsModularPropReqDTO.setModularId(cmsModularPropVO.getModularId());
        	cmsModularPropReqDTO.setStatus(CmsConstants.ParamStatus.CMS_PARAMSTATUS_1);
        	List<CmsModularPropRespDTO> list=this.iCmsModularPropRSV.queryCmsModularPropList(cmsModularPropReqDTO);
        	int sortNo=0;//取排序号
        	String[] ids=cmsModularPropVO.getSelectPropIds().split(",");
        	for (CmsModularPropRespDTO modularPropRespDTO:list) {
        		if(modularPropRespDTO.getSortNo()>sortNo){
        			sortNo=modularPropRespDTO.getSortNo();
        		}
			}
        	List<CmsModularPropReqDTO> addList=new ArrayList<>();
        	for (int i = 0; i < ids.length; i++) {
        		boolean isExist=false;
        		for (CmsModularPropRespDTO modularPropRespDTO:list) {
        			if(modularPropRespDTO.getPropId().toString().equals(ids[i])){
						isExist=true;
						break;
					}
        		}
        		if(!isExist){
	        		CmsModularPropReqDTO req = cmsModularPropVO.toBaseInfo(CmsModularPropReqDTO.class);
	        		req.setModularId(cmsModularPropVO.getModularId());
	        		req.setPropId(Long.valueOf(ids[i]));
	        		req.setStatus(CmsConstants.ParamStatus.CMS_PARAMSTATUS_1);
	        		req.setSortNo(++sortNo);
	        		addList.add(req);
        		}
			} 
        	this.iCmsModularPropRSV.addCmsModularPropList(addList);
    ...
    @ResponseBody
    public Model gridlist(Model model,CmsModularPropVO cmsModularPropVO) throws Exception{

    	CmsModularPropReqDTO req = cmsModularPropVO.toBaseInfo(CmsModularPropReqDTO.class);
    	req.setModularId(cmsModularPropVO.getModularId());
    	req.setStatus(cmsModularPropVO.getStatus());
    	PageResponseDTO<CmsModularPropRespDTO> pageInfo =iCmsModularPropRSV.queryCmsModularPropPage(req);
    	List<CmsModularPropRespDTO> resps=pageInfo.getResult();
    	if(CollectionUtils.isNotEmpty(resps)){
    		for(CmsModularPropRespDTO resp:resps){
    			CmsModularParaPropReqDTO modularParaPropReq=new CmsModularParaPropReqDTO();
    			modularParaPropReq.setId(resp.getPropId());
    			CmsModularParaPropRespDTO modularParaPropResp=iCmsModularParaPropRSV.queryCmsModularParaProp(modularParaPropReq);
    			resp.setPropName(modularParaPropResp.getPropName());
    			resp.setPropValueTypeZH(modularParaPropResp.getPropValueTypeZH());  //类型名称
    		}
    	}
    	EcpBasePageRespVO<Map> respVO = EcpBasePageRespVO.buildByPageResponseDTO(pageInfo);
        return super.addPageToModel(model, respVO);
    ...
    public String editView(Model model,CmsModularParaPropVO cmsModularParaPropVO,String isRead,String isValid,
            ...
            @ModelAttribute("searchModularParams") String searchModularParams,Long modularId) throws Exception{
    	boolean edit=false;
    ...
    @ResponseBody
    public EcpBaseResponseVO isCancle(Model model,CmsModularPropVO cmsModularPropVO) throws Exception{
    	EcpBaseResponseVO ecpBaseResponseVO = new EcpBaseResponseVO();
        try {
			iCmsModularPropRSV.changeStatusCmsModularProp(cmsModularPropVO.getId().toString(),ParamStatus.CMS_PARAMSTATUS_2);
        } catch (BusinessException e) {
            LogUtil.error(MODUAL, "操作失败！", e);
            ecpBaseResponseVO.setResultFlag(EcpBaseResponseVO.RESULT_FLAG_FAILURE);
            ecpBaseResponseVO.setResultMsg("操作失败！");
