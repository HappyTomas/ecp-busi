package com.zengshi.ecp.busi.order.ordback.controller;
...
import com.zengshi.ecp.busi.order.ordback.vo.RBackConfirmReqVO;
import com.zengshi.ecp.busi.order.ordback.vo.RBackGdsReviewReqVO;
import com.zengshi.ecp.busi.order.ordback.vo.ROrdBackReqVO;
import com.zengshi.ecp.busi.order.ordback.vo.ROrderBackVO;
...
//	    PageResponseDTO<RShopOrderResponse> resp = JSON.parseObject(val,new TypeReference<PageResponseDTO<RShopOrderResponse>>(){});
    ...
    public String backGdsReview(Model model, @Valid RBackGdsReviewReqVO vo) {
        ...
        ROrdReturnRefundResp rOrdReturnRefundResp = orderBackRSV.calCulateBackInfo(rOrderBackReq);
...
//        model.addAttribute("rBackGdsResps", resp.getrBackGdsResps());
    ...
    @ResponseBody
    public EcpBaseResponseVO modifyBackMoney(@Valid ROrderBackVO vo) {
        EcpBaseResponseVO resp = new EcpBaseResponseVO();
        ROrdReturnRefundReq req = new ROrdReturnRefundReq();
        ObjectCopyUtil.copyObjValue(vo, req, "", false);
        try{
        	orderBackRSV.modifyBackMoney(req);
            resp.setResultFlag(EcpBaseResponseVO.RESULT_FLAG_SUCCESS);           
        }catch(Exception e){
            e.printStackTrace();
            LogUtil.error(MODULE, "============出错了============="+e.getMessage());
    ...
    public String goModifyMoney(Model model, @Valid ROrderBackVO vo) {
         LogUtil.info(MODULE, JSON.toJSONString(vo).toString());
         model.addAttribute("orderBackVO",vo);
         return "/order/ordback/modify/modifymoney";
    }
    
    @RequestMapping(value="/checkBackMoney")
    @ResponseBody
    public EcpBaseResponseVO checkBackMoney(ROrderBackVO vo){
        LogUtil.info(MODULE, "---------------"+JSON.toJSONString(vo));
        EcpBaseResponseVO resp = new EcpBaseResponseVO();
        try{
        	ROrdReturnRefundReq req = new ROrdReturnRefundReq();
            ObjectCopyUtil.copyObjValue(vo, req, "", false);
            boolean result = ordMainRSV.checkBackMoney(req);
            if(result){
            	resp.setResultFlag(EcpBaseResponseVO.RESULT_FLAG_SUCCESS);
            }else{
