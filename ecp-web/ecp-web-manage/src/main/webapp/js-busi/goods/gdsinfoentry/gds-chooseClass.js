$(function(){
	...
	$("#btn_next").die('click').live('click',function(){
...
var GdsClass = {
		queryClassShopId : function(shopId){
			...
			param.shopId = shopId;
			$.eAjax({
				url : GLOBAL.WEBROOT + "/gdsinfoentry/getCategorysByShop",
				data : param,
				dataType:'html',
		...
		queryClass : function(){
			$.eAjax({
				url : GLOBAL.WEBROOT + "/gdsinfoentry/queryclass",
				data : param,
				success : function(returnInfo) {
		...
		getNextNode : function(catgCode){
			...
			_chooseFlag = false;
			$.eAjax({
				url : GLOBAL.WEBROOT + "/gdsinfoentry/getnextnode",
				async : true,
					...
					if(returnInfo.ecpBaseResponseVO.resultFlag=='ok'){
						 for(var i= 0;i<size;i++){
			            	html+= "<li id='"+returnInfo.nextNodeList[i].catgCode+"' onclick=\"checkedCat(this,'"+returnInfo.nextNodeList[i].catgCode+"')\" class='listNode' name='catgCode"+catgCode+"' catgName='"+returnInfo.nextNodeList[i].catgName+"'>" +
		                     	"<div class='d-text' title="+returnInfo.nextNodeList[i].catgName+" data-id='xx12' id='"+returnInfo.nextNodeList[i].catgCode+"'><p style='overflow: hidden; white-space: nowrap; text-overflow: ellipsis;width:120px;' > "+returnInfo.nextNodeList[i].catgName+"</p></div>" +
		                     	"</li>" ;
			             }
