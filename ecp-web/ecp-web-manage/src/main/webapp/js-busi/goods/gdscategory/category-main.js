$(function(){
	...
	var GdsCatgConstants = {
			updateUrl : GLOBAL.WEBROOT + "/goods/category/categoryupdate",
		    saveUrl : GLOBAL.WEBROOT + "/goods/category/categorysave",
		    ...
		    _addGroupsUrl : GLOBAL.WEBROOT+"/goods/category/addgroups",
	        _delGroupsUrl : GLOBAL.WEBROOT+"/goods/category/delgroups",
	        _addPropsUrl : GLOBAL.WEBROOT+"/goods/category/addprops",
	        _delPropsUrl : GLOBAL.WEBROOT+"/goods/category/delprops",
	        _propchangeUrl : GLOBAL.WEBROOT+"/goods/category/propchange",
  ...
  function ajaxDataFilter(treeId, parentNode, responseData) {
	...
	var actionListener = {
		...
		delCategory : function(){
	    	...
	    	eDialog.confirm('您确认要删除选定'+GdsCatgConstants.getCatgTypeName()+'吗?',{
				buttons:[{
					...
					callback:function(){
				    	$.eAjax({
							url : GLOBAL.WEBROOT + "/goods/category/categorydel",
							type : "POST",
		...
		editCheck : function(catgCode){
			...
			///////分类删除检测开始。
			$.eAjax({
				url : GLOBAL.WEBROOT + '/goods/category/editCheck',
				data : {
					'catgCode' : catgCode
				},
		...
		propDelCheck : function(id){
			...
			///////分类属性关联关系删除检测开始。
			$.eAjax({
				url : GLOBAL.WEBROOT + '/goods/category/propDelCheck',
				data :datas,
				success : function(returnInfo) {
					if(null != returnInfo 
							&& null != returnInfo.ecpBaseResponseVO
		...
		imgUpload : function(e){
			busSelector.uploader({
				...
				// uploadUrl : GLOBAL.WEBROOT + "/goods/category/imgUpload",
				// uploadFileObjName : 'imgObj',
				fileSizeLimit : "100KB",
				checktype : "single",
				callback : function(data){
					if(data && data.results && data.results.length > 0){
						var result = data.results[0];
						$('#mediaUuid').val(result.fileId);
	...
	var _initOptGroupsDT = {
			...
			'pCheck' : 'multi',
			'pSingleCheckClean' : true,
			"sAjaxSource" : GLOBAL.WEBROOT + '/goods/category/listoptgroups',
			// 指定列数据位置
			"aoColumns" : [{
						"mData" : "id",
						"sTitle" : "属性组编码",
	...
	var _initConfigedPropGridDT = {
			'pTableTools' : false,
			'pSingleCheckClean' : true,
			"sAjaxSource" : GLOBAL.WEBROOT + '/goods/category/listconfigedprop',
			'pCheck' : 'multi',
			// 指定列数据位置
			"aoColumns" : [{
						"mData" : "propId",
...
//					}
	...
	var _initOptPropGridDT = {
			...
			'pCheck' : 'multi',
			'pSingleCheckClean' : true,
			"sAjaxSource" : GLOBAL.WEBROOT + '/goods/category/listoptionalprop',
			// 指定列数据位置
			"aoColumns" : [{
						"mData" : "id",
						"sTitle" : "属性编码",
