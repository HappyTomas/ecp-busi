...
//	});
	...
	var importTempData = function(importUuid, importFile) {
		...
		$.eAjax({
					url : GLOBAL.WEBROOT
							+ "/goods/excelEditImport/importTempData",
					data : {
						"importId" : importId,
					...
					success : function(data) {
						...
						var remsg = data.resultMsg;
						if (reflag == "ok") {
							eDialog.success("导入数据成功！一共导入" + data.totalCount
									+ "条,其中成功" + data.successCount + "条,失败"
									+ data.failCount + "条");
						} else {
...
//		'uploader' : $webroot + 'goods/excelEditImport/uploadfile;'+ $('#pageSessionId').val(),// 后台接收文件处理的controller
//		// 'uploader' : $webroot + 'gdsprom/uploadfile',//后台接收文件处理的controller
...
//	});
	$("#uploadFileButtton").eUploadBaseInit({
		uploadUrl : $webroot + 'goods/excelEditImport/uploadfile;'+ $('#pageSessionId').val(),// 后台接收文件处理的controller
	...
	$("#dataGridTableTemp").initDT({
		...
		'pCheckColumn' : false,
		"sAjaxSource" : GLOBAL.WEBROOT + '/goods/excelEditImport/importLogList',
		'params' : [{
					name : 'importId',
					value : $('#importId').val()
				}],
...
var gdsExcelImportData = {
		...
		eDialog.confirm("您确认删除该数据吗？", {
			buttons : [{
				...
				callback : function() {
					$.eAjax({
								url : GLOBAL.WEBROOT
										+ "/goods/excelEditImport/delImportLog",
								data : {
	...
	listResult : function(importId) {
		bDialog.open({
					...
					height : 500,
					url : GLOBAL.WEBROOT
							+ "/goods/excelEditImport/watchResultList",
