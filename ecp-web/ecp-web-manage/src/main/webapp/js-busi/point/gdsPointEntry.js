    ...
    $("#saveGdsInfo").click(function(){
    	...
    	if(!$("#priceSettingForm").valid()){
    		$body = (window.opera) ? (document.compatMode == "CSS1Compat" ? $('html')
    				...
    				: $('html,body');
    		$body.animate({
    			scrollTop : $("#pointErrorPlace").offset().top
    		}, "fast");
    		return;
    	} 
    	if(!$("#gdsSkuForm").valid()){
...
//					}
		...
		checkSkuParameter : function (propId, propName, propValueId, propValue,currentIndex,indexCount,obj) {
			if($("#sku-param-table").find('tr').length==0){
                ...
                "<tr>"+
                    ...
                    "<th id='getPriceTh'>操作</th>"+
                "</tr>"+
                "</thead>"+
                "<tbody>"+
                "<tr id='esp'>"+
                    "<td style='display:none;' id='senior-price'>"+pricehtml+"</td>"+
                    "<td style='display:none;' id='senior-stock'>"+stockhtml+"</td>"+
	    ...
	    getPramaListTdHtml : function () { 
	    	tagIdCount++;
	    	var tdHtml = "<td style='display:none;' class='gdsNameTd' id='senior-price'>" + GdsInfoEntry.getInputHtml("price", tagIdCount);
	    	tdHtml += "</td>" + "<td style='display:none;' id='senior-stock'>" + GdsInfoEntry.getInputHtml("stock", tagIdCount);
...
//	    	}
	    ...
	    openStockWin : function(obj){
    	        bDialog.open({
    	            ...
    	            width : 680,
    	            height : 400,
    	            url :  GLOBAL.WEBROOT + "/gdspointentry/toseniorstock?shopId="+$("#shopId").val()+"&&editFlag='0'",
    	            params : {
...
//		},
		...
		saveGdsInfo : function(obj){
			...
			$.eAjax({
				url : GLOBAL.WEBROOT + "/gdspointentry/savegdsinfo",
				data :param,
...
//			param.ifEntityCode = $("#ifEntityCode").val();
		...
		getScorePriceParam : function(){
			...
			$("#lader-price-table").find("tr").each(function(){
				...
				scorePriceParam += "{id:'',scoreType:'"+type+"',score:'"+score+"',scorePrice:'"+scorePrice+"',ifDefault:'"+ifDefault+"'},";
			});
			scorePriceParam += "]";
		...
		getSkuParam : function (){
			...
			if(multi_switch=='0'){
				var defaultSku = "[{ifHaveto:'',ifBasic:'',propId:'',propValueId:'',propName:'',propValue:''" +
						",skuPriceList:[],skuStockList:"+skuStockList+",skuPictrueList:[],skuAmount:'1',skuId:''}]";
				return defaultSku;
			}
			// 初始化单品参数
			var skuParam = "[";
				...
				if (index == 0) {
					...
					$(this).find(".property").each(function(index, element) {
						...
						skuParam = skuParam + "{ifHaveto:'"+ifHaveto+"',ifBasic:'"+ifBasic+"',propId:'" + propId + "',propValueId:'"
								...
								+ gdsValue + "',skuPriceList:[],skuStockList:"+skuStockList+",skuPictrueList:"+skuPictrueList+",skuAmount:'"+skuAmount+"',skuId:''},";
						count ++;
					});
					if(count == 0){
						skuParam = skuParam + "{propId:'',propValueId:'',propName:'',propValue:'',skuPriceList:[],skuStockList:[],skuPictrueList:"+skuPictrueList+",skuAmount:'1',skuId:''},";
					}
				}
			});
			skuParam = skuParam + "]";
