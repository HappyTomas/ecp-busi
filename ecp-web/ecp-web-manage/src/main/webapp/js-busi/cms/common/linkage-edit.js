...
var linkage_edit = {
	...
	initSite : function(){
		$.eAjax({
			...
			success : function(returnInfo){
				...
				linkage_edit.changeSite(true);
			}	
		});
	},
	changeSite : function(isInit){//站点来匹配模板
		if(!isInit){
			$('#catg-code').val('');//清空关联商品分类
			$('#catg-code-name').val('');//清空关联商品分类
		}
		$('#templateId').empty();
	...
	changeTemplate : function(){//模板来匹配内容位置
		...
		if(templateId != "" && templateId != null){
			$.eAjax({
				url : $webroot + 'common/changeTemplate',
				data : {
