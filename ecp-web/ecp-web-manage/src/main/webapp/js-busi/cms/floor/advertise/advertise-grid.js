...
var advertise = {
	init : function(){//初始化
		...
		$("#dataGridTable").initDT({
	        ...
	        'pCheck' : "multi",
	        'params' : params,
	        "sAjaxSource": $webroot + 'flooradvertise/gridlist?floorId='+floorId,
	        //指定列数据位置
	        "aoColumns": [
	        	{ "mData": "id", "sTitle":"id","bVisible":false,"sWidth":"80px","bSortable":false},
				{ "mData": "advertiseTitle", "sTitle":"广告语","sWidth":"220px","bSortable":false},
				...
				{ "mData": "floorId", "sTitle":"所属楼层ID","bVisible":false,"bSortable":false},
				...
				{"targets": -1,"mData": "id","sTitle":"操作","sWidth":"180px","bSortable":false,"sClass": "left",
					"mRender": function(data,type,row){
						//alert(data+"---"+type+"---"+row);
						var optionview=" <a href='javascript:void(0)' onclick='advertise.viewFloorAdvertise("+row.id+")'>查看</a>";
						if(row.status == "0"){//无效
							return optionview+" | <a href='javascript:void(0)' onclick='advertise.changeStatus("+data+",\"1\")'>发布</a> | <a href='javascript:void(0)' onclick='advertise.edit("+data+")'>编辑</a> | <a href='javascript:void(0)' onclick='advertise.deleteByID("+data+")'>使失效</a>";
						}else if(row.status == "1"){//有效
							return optionview+" | <a href='javascript:void(0)' onclick='advertise.changeStatus("+data+",\"0\")'>撤消</a>";
						}else{//使失效
							return optionview+"";
						}
					}
		...
		$('#btn_code_add').click(function(){
			...
			SearchObj.openPage({
				"url" : $webroot+'flooradvertise/add',
				"params" : params,
				"method" :"post"
			});
		});
	...
	viewFloorAdvertise : function(id){
		eNav.setSubPageText('查看楼层广告');
		var searchParams = SearchObj.getFormParam($("#searchForm"));
		var params = {
			"id":id,
			"searchParams":searchParams
		...
		SearchObj.openPage({
			"url" : $webroot+'flooradvertise/view',
			"params" : params,
			"method" :"post"
		});
	},
	edit : function(id){   //编辑
		...
		SearchObj.openPage({
			"url" : $webroot+'flooradvertise/edit',
			"params" : params,
			"method" :"post"
		});
	},
	deleteByID : function(id){   //使失效
		...
		eDialog.confirm("您确认使失效该记录吗？", {
			buttons : [{
				...
				callback : function(){
					$.eAjax({
						url : $webroot + 'flooradvertise/delete',
						type : "POST",
						data : param,
						datatype:'json',
						beforeSend:function(){
	...
	changeStatus : function(id,status){ //生效、失效
		...
		$.eAjax({
			url : $webroot + "flooradvertise/changestatus",
			type : "POST",
			data : param,
			datatype:'json',
			beforeSend:function(){
