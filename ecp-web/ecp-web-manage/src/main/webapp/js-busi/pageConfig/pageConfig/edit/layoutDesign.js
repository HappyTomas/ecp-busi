...
var layoutDesign = {
    ...
    "pubLock":0,
	"init":function(){
		layoutDesign.layoutAdminCount = $(".layoutAdmin",".layoutAdmin-bd").length;
		layoutDesign.resortLayout();//布局重新排序
		layoutDesign.initLayoutAdminCss();//初始化布局宽度样式
		layoutDesign.initLayoutTypeCss();//初始化布局类型宽度样式
		layoutDesign.initLayoutDialog();//初始化布局类型弹出框事件
		layoutDesign.bandLayoutAdmin();//初始化布局事件
		layoutDesign.bandLayoutType();//初始化布局类型事件
		
	},
	"bandLayoutAdmin":function(){//绑定布局实体的交互事件
		layoutDesign.bandAddLayoutAdmin();//初始化添加布局事件
		layoutDesign.bandDelLayoutAdmin();//初始化删除布局事件
		layoutDesign.bandEditLayoutAdmin();//初始化替换布局事件
		layoutDesign.bandMoveLayoutAdmin();//初始化移动布局事件
		
		layoutDesign.bandDelLayoutItem();//初始化删除布局项事件
		layoutDesign.initLayoutItemSort();//初始化布局项排序    将错误的排序更正过来
	},
	/**
	 * 布局拖拽排序  start
	 */
	"bandMoveLayoutAdmin":function(){
		$("div.layoutAdmin-bd").dragsort({ 
			dragSelector: "a.move", 
			dragBetween: false, 
			dragEnd: layoutDesign.resortLayout, 
	...
	"resortLayout":function(){
		...
		$layoutAdminList.each(function(i,el){
			...
			if(showOrder != (i+1)){
				...
				param.layoutList.push(delLayout);
			}
		});
		
		if(param.layoutList.length > 0){
			//异步更新
			$.eAjax({
				url : $webroot + "layoutDesign/updateLayoutBatch",
	...
	"bandEditLayoutAdmin":function(){
		$('.layoutAdmin .edit').die().live("click", function() {
			...
			var callback = function(data){
				layoutDesign.editLayoutDialogCallback($layoutAdmin,data);
			}
			if(layoutTypeId){
		...
		if(($layoutAdmin instanceof jQuery) && returnData &&returnData.results && returnData.results[0]){
			if(returnData.results[0].resultFlag === true && returnData.results[0].param.layoutTypeId != $layoutAdmin.attr("data-layout-type-id")){
				...
				var successCallback = function(data){
					...
					layoutDesign.initLayoutAdminCss();
					//刷新拖拽功能作用对象
				...
				var doEdit= function(){
					...
					layoutDesign.updateLayoutAdmin(returnData.results[0].param,successCallback);	
				}
	...
	"updateLayoutAdmin":function(param,callback){//保存布局与布局项
		if(param && param.layoutTypeId){
			$.eAjax({
				url : $webroot + "layoutDesign/updateLayout",
				data : param,
				success : function(returnInfo){
					if(returnInfo && returnInfo.resultFlag == 'ok'){//成功
	...
	"bandAddLayoutAdmin":function(){
		...
		$('#addLayout').die().live("click",function(){
            ...
            bDialog.open({
                ...
                width : 520,
                height : 400,
                callback:layoutDesign.addLayoutDialogCallback,
                onShow:function(){
	...
	"addLayoutDialogCallback":function(returnData){//添加布局弹出框回调函数
		if(returnData &&returnData.results && returnData.results[0]){
			if(returnData.results[0].resultFlag === true){
				if(layoutDesign.addDelLock > 0 ){//锁定处理
					...
					if(returnData.results[0].param.layoutShowType == "03" || returnData.results[0].param.layoutShowType == "04"){
						if($(".layoutAdmin-float[data-layout-type-id="+returnData.results[0].param.layoutTypeId+"]").length <= 0){//悬浮只能有一个
							layoutDesign.addDelLock += 1;//加锁
							
							//回调函数
							var successCallback = function(data){
								layoutDesign.addDelLock -= 1;//解锁
							...
							layoutDesign.saveLayoutAdmin(returnData.results[0].param,successCallback,errorCallback);
						}
					}else{//普通布局
						layoutDesign.addDelLock += 1;//加锁
						returnData.results[0].param.showOrder = layoutDesign.layoutAdminCount + 1;
						
						//回调函数
						var successCallback = function(data){
							layoutDesign.addDelLock -= 1;//解锁
							layoutDesign.layoutAdminCount += 1;
							
							var html = template("layoutAdmin-template", data);//使用arttemplate插件
							$(".layoutAdmin-bd").append(html);
							
							//如果页面只有一个布局则刷新排序事件
							if(layoutDesign.layoutAdminCount == 1){
								layoutDesign.bandMoveLayoutAdmin();
							}
							layoutDesign.initLayoutAdminCss();
							//刷新拖拽功能作用对象
							if(doJsmartdrag && doJsmartdrag.jsmartdragObj){
	...
	"saveLayoutAdmin":function(param,successCallback,errorCallback){//保存布局与布局项
		...
		if(param && param.layoutTypeId){
			$.eAjax({
				url : $webroot + "layoutDesign/saveLayout",
				data : param,
				success : function(returnInfo){
					if(returnInfo && returnInfo.resultFlag == 'ok'){//成功
			...
			if(layoutDesign.addDelLock > 0 ){//锁定处理
				...
				var successCallback = function(){
					if(!$layoutAdmin.hasClass("layoutAdmin-float")){//非悬浮布局
						layoutDesign.layoutAdminCount -= 1;//将布局个数减1，新增才不会出错
					}
					layoutDesign.addDelLock -= 1;//解锁
					//刷新拖拽功能作用对象
					if(doJsmartdrag && doJsmartdrag.jsmartdragObj){
				...
				var doDel = function(){
					...
					layoutDesign.delLayoutAdmin($layoutAdmin,successCallback,errorCallback);
				}
	...
	"delLayoutAdmin":function($layoutAdmin,successCallback,errorCallback){//删除布局并重排
		...
		if($layoutAdmin instanceof jQuery ){
			...
			if(delLayout.id){
				...
				//异步更新
				if(param && param.layoutList.length > 0){
					$.eAjax({
						url : $webroot + "layoutDesign/updateLayoutBatch",
						data : ebcUtils.serializeObject(param),
						success : function(returnInfo){
...
//---------------------布局项相关--------------------------//
	"initLayoutItemSort":function(){
		...
		$layoutLists.each(function(i,el){
			...
			layoutDesign.resortLayoutItem($this,null,successCallback,errorCallback);
		});
	},
	"saveLayoutItem":function(itemParam,successCallback,errorCallback){
		...
		if(itemParam && itemParam.layoutId && itemParam.itemSize && itemParam.itemNo && itemParam.rowNo){
			//异步更新
			$.eAjax({
				url : $webroot + "layoutDesign/saveLayoutItem",
				data : itemParam,
				success : function(returnInfo){
	...
	"updateLayoutItem":function($item,options,successCallback,errorCallback){
		...
		if($item instanceof jQuery){
			...
			if(param && param.id){
				//异步更新
				$.eAjax({
					url : $webroot + "layoutDesign/updateLayoutItem",
					data : param,
					success : function(returnInfo){
						if(returnInfo && returnInfo.resultFlag == 'ok'){//成功
							successCallback();
	...
	"updateLayoutItemBatch":function(param,successCallback,errorCallback){
		...
		if(param.layoutItemList.length > 0){
			//异步更新
			$.eAjax({
				url : $webroot + "layoutDesign/updateLayoutItemBatch",
				data : ebcUtils.serializeObject(param),
				success : function(returnInfo){
					if(returnInfo && returnInfo.resultFlag == 'ok'){//成功
						successCallback();
	...
	"resortLayoutItem":function($itemList,param,successCallback,errorCallback){
		...
		$layoutItemList.each(function(i,el){
			...
			if(rowNo != (i+1)){
				...
				param.layoutItemList.push(layoutItem);
			}
		});
		
		//保存数据
		if(param.layoutItemList.length > 0){
			layoutDesign.updateLayoutItemBatch(param,successCallback,errorCallback);
		}else{
			successCallback();
		}
		...
		$(".close",".item-admin").die().live("click",function(){
			...
			if(lock){
				...
				if($itemList.length > 0){//多个布局项则删除改布局
					...
					layoutDesign.resortLayoutItem($(this).parents(".item-list"),param,successCallback,errorCallback);
	...
	"initLayoutAdminCss":function(){
		...
		$layoutAdmins.each(function(){
             ...
             numList = layoutDesign.numToTrueWidth(numList,5,$this.width());
	...
	"numToTrueWidth":function(numList,blank,totalWidth){
		...
		if(layoutDesign.isEmptyList(numList) || numList.length == 1 || !totalWidth){
			return [totalWidth];
		}
