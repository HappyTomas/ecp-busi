$(function(){
	var siteId = $('#siteId',"#homepage").val();
	var pageTypeId = $('#pageTypeId',"#homepage").val();
	var url = 'cmslinkinpututil/pagegrid?siteId='+(siteId||"")+"&pageTypeId="+(pageTypeId||51);
	//初始化列表
	$("#dataGridTable","#homepage").initDT({
        'pTableTools' : false,
	...
	//绑定查询按钮
	$('#btnFormSearch',"#homepage").click(function(){
		if(!$("#searchForm","#homepage").valid()) return false;
		var p = ebcForm.formParams($("#searchForm","#homepage"));
		$('#dataGridTable',"#homepage").gridSearch(p);
	});
	//绑定重置按钮
	$('#btnFormReset',"#homepage").click(function(){
		ebcForm.resetForm('#searchForm',"#homepage");
	});
});
