$(function(){
	pageConfigSideBarWap.init();
});
var pageConfigSideBarWap = {
		"init" : function(){//初始化左侧栏
			pageConfigSideBarWap.bandSelectSidebarItem();
			pageConfigSideBarWap.bandCloseSidebarItem();
			pageConfigSideBarWap.getModule($(".layout-item span.size-selected").text());
			pageConfigSideBarWap.bandSelectItemSize();
			pageConfigSideBarWap.initPageAttrbute();//初始化页面属性交互事件
		},
		"initPageAttrbute":function(){//初始化页面属性交互事件
			//绑定背景色   （背景色，是否显示）
			pageConfigSideBarWap.bandBgColor();
			//绑定背景图    （ 更改    删除     展示方式）
			pageConfigSideBarWap.bandBgPic();
		...
		"bandSelectItemSize" : function(){//绑定选择布局规格交互事件
			...
			$(".layout-item span").live("click",function(){
				if(!$(this).hasClass("size-selected")){
					...
					pageConfigSideBarWap.getModule($(this).text());
				}
			});
		},
		"bandBgColor" :function(){
			...
			pageConfigSideBarWap.bandJpicker($bgColor);//绑定取色器
			//绑定是否显示背景色交互事件
			$("#attr-showBackFlag").click(function(){
				...
				//保存
				pageConfigSideBarWap.saveBgColor();
			});
		...
		"bandBgPic" : function(){
			...
			var updateImgCallback = function(returnInfo) {
	    		...
	    		if (returnInfo && returnInfo.success == "ok") {
	    			if(returnInfo.resultMap && returnInfo.resultMap.vfsUrl && returnInfo.resultMap.vfsId){
		    			...
		    			//保存背景图
		    			pageConfigSideBarWap.saveBgPic();
	    			}else{
			...
			$("#change-bg-img").live("change", function(o) {
			    ...
			    if(path==""){
			    	...
			    	pageConfigSideBarWap.uploadImage(this, path,updateImgCallback);
			    }
			});
		...
		"getModule":function(layoutItemType){//获取模块
			if(!pageConfigSideBarWap.isPositiveNum(layoutItemType)){
				layoutItemType = "";
			}
						...
						if(returnInfo.modulars && returnInfo.modulars.length && returnInfo.modulars.length > 0){
							...
							if(doJsmartdrag){
								doJsmartdrag.bandJsmartdrag($(".module-list li"),{//拖拽参数
									...
									afterDrag : pageConfigSideBarWap.modularAfterDrag
								});
							}
						}else{
		...
		"modularAfterDrag":function (selected, $current, $target) {
			...
			var hiddenRowNo ;
			var moduleData =  pageConfigSideBarWap.getData($current);//获取模块的属性数据
			var $lastOne=$target.prev('.phone-layout');
			var $nextOne=$target.next('.phone-layout');
			if($nextOne.length!=0 ){
				if( moduleData.id==111 ||  moduleData.id==116 ||  moduleData.id==117){
					eDialog.alert("该模块只能放在最下方");
					return;
				}
			}
			var last_modularId=$('#modularId',$lastOne).val();
			if(111==last_modularId || 116==last_modularId || 117==last_modularId){
				eDialog.alert("上方模块只能放在最下方，请重新配置");
				return;
			}
			var menu = $('<div class="menu"><span class="mup"></span><span class="medit"></span><span class="mdown"></span><span class="mdel"></span></div>');
			...
			layoutItem.pageId = $('#pageId').val();
			layoutItem.layoutId = $('#layoutId').val();
			layoutItem.modularId = moduleData.id;
			layoutItem.modularType = moduleData.modularType;
			layoutItem.componentEditUrl = moduleData.componentEditUrl||"";
			layoutItem.componentVmUrl = moduleData.componentVmUrl||"";
			...
			hiddenModularEditUrl=$('<input type="hidden" value="'+layoutItem.componentEditUrl+'" id="componentEditUrl"/>');
			hiddenComponentVmUrl=$('<input type="hidden" value="'+layoutItem.componentVmUrl+'" id="componentVmUrl"/>');
			//回调函数
			var successCallback = function(itemData){
				...
				.append(hiddenModularEditUrl)
				.append(hiddenComponentVmUrl)
				.append(hiddenRowNo)
				.append(menu)
				.append(phone_modular)
				.attr("id","item-"+itemData.id);
...
//				//刷新拖拽功能作用对象
			...
			//保存数据
			pageConfigSideBarWap.saveLayoutItem(layoutItem,successCallback,errorCallback);
			var param = {
				"layoutItemList":[]
			};
		...
		"savePageAttrbute": function(options,callback){
			...
			if(pageConfigSideBarWap.isEmptyList(options)){
				options = ["backgroundColor","backgroundPic","showBackFlag","backgroupShowType","matchingColour"];
			}
		...
		"saveBgColor" : function(){//保存背景颜色   包括背景色与是否显示背景色
			...
			pageConfigSideBarWap.savePageAttrbute(options, pageConfigSideBarWap.showBgColor);
		},
		...
		"saveBgPic" : function(){
			...
			pageConfigSideBarWap.savePageAttrbute(options, pageConfigSideBarWap.showBgPic);
		},
...
//--------------------------------工具方法-----------------------------------//
		...
		"getData":function($obj,str){
			if($($obj).length > 0){
				if(str){
					str = String(str);
					str = pageConfigSideBarWap.jsAttrToHtmlAttr(str);
					return $obj.attr(str);
				}else{
					var result = {};
						...
						$.each(attributes,function(i,attribute){
							...
							if(attrVal){
								if(reg.test(attrName)){
									attrName =  pageConfigSideBarWap.htmlAttrToJsAttr(attrName);
		...
		"setData":function($obj,name,value){
			...
			if($($obj).length > 0){
				name = pageConfigSideBarWap.jsAttrToHtmlAttr(name);
				$obj.attr(name,value);
			}
		},
