$(function(){
	modularComponent_edit.init();
});

var modularComponent_edit = {
	init : function(){//初始化
		if($('#isRead').val()=='1'){
		...
		$('#modularName').live('blur',function(){
			modularComponent_edit.sameModular($(this));
		});
		//发布按钮
//		$('#btnPublish').click(function(){ 
//			modularComponent_edit.saveFrom("1");
//		});
		//保存按钮
		$('#btnSave').click(function(){ 
			modularComponent_edit.saveFrom();
		});
		//退回
		$('#btnReturn').click(function(){ 
			var modularId=$('#modularId').val();
			window.location.href = GLOBAL.WEBROOT+'/modularComponent/init?modularId='+modularId+'&isValid=0';
		});
		//选择页面类型
		$('#select_PageType').click(function(){ 
			modularComponent_edit.openPageType();
		});
		//选择布局项尺寸 
		$('#select_ItemSize').click(function(){ 
			modularComponent_edit.openItemSize();
		});
		//选择关联组件 
		$('#select_componentId').click(function(){ 
			modularComponent_edit.openComponent();
		});
		//点击适用页面类型事件
		$("#applyPageType").live("change", function(o) {
			$("#applyItemSize").val('');
		});
		//点击图片上传触发事件
		$("#uploadFileObj").live("change", function(o) {
		    ...
		    if(path==""){
		    	...
		    	modularComponent_edit.uploadImage(this, path);
		    }
		});
		//清除图片
		$('#clean_image').click(function(){
			modularComponent_edit.cleanImage();
		});
	...
	openItemSize : function(){
		var applyPageType = $('#applyPageType').val();		
		//var linkType = $("input[name=linkType]:checked").val();
		if(!applyPageType || applyPageType.length==0){
		...
		bDialog.open({
			...
			width : 460,
			height : 300,
			url : $webroot + "modular/itemSizeView?applyItemSize="+$("#applyItemSize").val()+"&applyPageTypeId="+$('#applyPageType').val(),
			params : {
			},
			callback:function(data){
				var result=data?data.results:[];
	...
	saveFrom : function(doSuccess){//保存
		...
		$.eAjax({
			url : $webroot + "modularComponent/save",
			data : params,
			success : doSuccess,
			error : function(e,xhr,opt){
			...
			if(returnInfo.resultFlag == 'ok'){//保存成功
				eDialog.success('发布成功！',{
					buttons:[{
						caption:"确定",
						callback:function(){
							window.location.href = GLOBAL.WEBROOT+'/modular/init?modularId='+modularId+'&isValid='+isValid;
						}
					}]
				});
			}else{
	...
	uploadImage : function (object, path) {
    	...
    	modularComponent_edit.ajaxFileUpload(url, false, $(object).attr('id'), "POST", "json", callback,data);
    },
