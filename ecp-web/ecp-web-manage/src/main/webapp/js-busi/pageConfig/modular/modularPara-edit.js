$(function(){
	modularPara_edit.init();
});

var modularPara_edit = {
	init : function(){//初始化
		...
		$('#propName').live('blur',function(){
			if($('#isRead').val()=='1'||$('#isPubish').val()=='1'){
				...
				modularPara_edit.samePropName($(this));
			}
		});
		//发布按钮
		$('#btnPublish').click(function(){ 
			modularPara_edit.saveFrom("1");
		});
		//保存按钮
		$('#btnSave').click(function(){ 
			modularPara_edit.saveFrom();
		});
		if(modularPara_edit.isSubimitProp()){
		...
		$('#btnReturn').click(function(){ 
			window.location.href = GLOBAL.WEBROOT+'/modularPara/init?modularId='+$('#modularId').val()+'&isValid='+$('#isValid').val();
		});
	...
	samePropName:function($this){
		if($this.val()!=''){
			...
			$.eAjax({
				url : $webroot + "modularPara/samePropName",
				data : params,
				async:false,
				...
				if(returnInfo.resultFlag == 'ok'){//保存成功
					eDialog.success(successMsg,{
						buttons:[{
							caption:"确定",
							callback:function(){
								window.location.href = GLOBAL.WEBROOT+'/modularPara/init?modularId='+$('#modularId').val();
							}
						}]
					});
				}else{
	...
	"publishPage" : function(){
			...
			if(returnInfo.resultFlag == 'ok'){//保存成功
				eDialog.success('发布成功！',{
					buttons:[{
						caption:"确定",
						callback:function(){
							window.location.href = GLOBAL.WEBROOT+'/modularPara/init';
						}
					}]
				});
			}else{
		...
		$.gridLoading({"message":"正在加载中...."});
		$.eAjax({
			url : $webroot + "modularPara/publish",
