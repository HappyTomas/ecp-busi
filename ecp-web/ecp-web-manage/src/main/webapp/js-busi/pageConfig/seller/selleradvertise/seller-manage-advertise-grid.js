...
var advertise = {
	init : function(){//初始化
		...
		$("#dataGridTable").initDT({
	        ...
	        'pCheck' : "single",
	        'params' : params,
	        "sAjaxSource": $webroot + 'cmsselleradvertise/gridlist',
	        //指定列数据位置
	        "aoColumns": [
		...
		$('#btn_code_add').click(function(){
			...
			SearchObj.openPage({
				"url" : $webroot+'cmsselleradvertise/add',
				"params" : params,
				"method" :"post"
			});
		});
	...
	edit : function(id){   //编辑
		...
		SearchObj.openPage({
			"url" : $webroot+'cmsselleradvertise/edit',
			"params" : params,
			"method" :"post"
		});
	},
	deleteByID : function(id){   //使失效
		...
		eDialog.confirm("您确认使失效该记录吗？", {
			buttons : [{
				...
				callback : function(){
					$.eAjax({
						url : $webroot + 'cmsselleradvertise/delete',
						type : "POST",
						data : param,
						datatype:'json',
						beforeSend:function(){
	...
	changeStatus : function(id,status){ //生效、失效
		...
		$.eAjax({
			url : $webroot + "cmsselleradvertise/changestatus",
			type : "POST",
			data : param,
			datatype:'json',
			beforeSend:function(){
	...
	showByID : function (id) {
		...
		SearchObj.openPage({
			"url" : $webroot+'cmsselleradvertise/view',
			"params" : params,
			"method" :"post"
		});
	}
