...
var advertise_edit = {
	...
	init : function(){//初始化
		//将图片信息缓存起来
		advertise_edit.saveAdInfoCache();
		//绑定选择内容位置
		$('#placeId').bind('change', function(){
			var placeId = $('#placeId').val();
			if(advertise_edit.advertiseCache.placeId && placeId==advertise_edit.advertiseCache.placeId){
				$("#vfsName").val(advertise_edit.advertiseCache.vfsName);
				$("#vfsId").val(advertise_edit.advertiseCache.vfsId);
				$("#imagePreview").attr("src",advertise_edit.advertiseCache.imagePreview);
			}else{
				$("#vfsName").val("");
				$("#vfsId").val("");
				$("#imagePreview").attr("src",advertise_edit.advertiseCache.emptyImageUrl);
			}
		...
		$('#btnReturn').click(function(){
			...
			SearchObj.openPage({
				"url": $webroot+'cmsselleradvertise/grid',
	...
	saveAdInfoCache:function(){
		var placeId = $("#linkage_placeId").val();
		if(placeId!=null && placeId!= (void 0) && placeId!=''){
			advertise_edit.advertiseCache.placeId = placeId;
			advertise_edit.advertiseCache.vfsName = $("#vfsName").val();
			advertise_edit.advertiseCache.vfsId=$("#vfsId").val();
			advertise_edit.advertiseCache.imagePreview=$("#imagePreview").attr("src");
		}
		advertise_edit.advertiseCache.emptyImageUrl =  $("#emptyImageUrl").val();
	},
	...
	openLinkDetail : function(){
		...
		if(linkType == "01"){//商品
			...
			url = "cmsselleradvertise/opengds?shopId="+shopId+"&siteId="+siteId;
		}else if(linkType == "02"){//公告
			url = "cmsselleradvertise/openinfo";
	...
	saveFrom : function(){//保存
		...
		$.eAjax({
			url : $webroot + "cmsselleradvertise/save",
	...
	pubSaveFrom : function(){//发布保存
		...
		$.gridLoading({"message":"正在加载中...."});
		$.eAjax({
			url : $webroot + "cmsselleradvertise/pubsave",
			data : ebcForm.formParams($("#detailInfoForm")),
			success : function(returnInfo) {
	...
	showUploadWarn : function(){//根据内容位置的限制控制上传图片提醒
		...
		if(placeValue == null || placeValue == ''){
			...
			if(place_width != '' && place_height != ''){
				var warnText = "温馨提醒：请上传小于"+place_size+"k的图片（jpg,png,jpeg,gif,bmp），规格为"+place_width+"*"+place_height+"px！";
