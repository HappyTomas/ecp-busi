...
(function(factory) {
		...
		define([], function() {
			...
			return {
				componentInit : function() {
					$(".modular-wap-coupon").modular_wap_coupon();
				}
			};
		});
	} else {
	...
	$modularWapCoupon = {
		...
		"control":function(el,opts){
			var coupIds = $modularWapCoupon.getCoupIds(el);
			if(coupIds){
				$.eAjax({
					...
					success : function(returnInfo) {
						if(returnInfo && returnInfo.resultFlag == 'ok'){
							$modularWapCoupon.doData(el,opts,returnInfo.coupons);
						}else{
		...
		"doData":function(el,opts,datas){
			...
			for(var i in datas){
				...
				if(+data.coupValue <= -1){
					$(".price-box",$item).html('<div class="price"> <span class="price-m">抵用券</span></div>');
				}else{
					...
					if(1 <= +useRuleObj["240"]){ // 折扣券
						$(".price",$item).html('<span class="price-m">'+(value/100000).toFixed(1)+'</span><span class="price-sign"> 折</span>')
						$(".while-q",$item).html("折扣券");
					}else{
						$(".price-m",$item).text(parseInt(value/10000));
					}
				}
				
				$(".user-tj",$item).text(data.conditionsShow||"");
			...
			if(opts && opts.isPub){
				...
				if(0 >= $(".usefull-coup",$el).length){
					...
					if(typeof opts.displayPlatType == 'string' && opts.displayPlatType.toLowerCase() == 'app'){//app访问则用a链接
						$modularWapCoupon.setHrefOfCoups(el,opts.webRoot);
					}else{//否则用点击事件
						$modularWapCoupon.bindGainCoup(el,opts.webRoot);
					}
				}
			}else{
		...
		"bindGainCoup":function(el,webRoot){
			...
			var doBind = function(){
				...
				$modularWapCoupon.gainCoup(webRoot,coupId,sCallback,eCallback,$(this));
			}
	...
	$.fn.modular_wap_coupon = function() {
		return this.each(function() {
			...
			$modularWapCoupon.control(this,opts);
