$(function(){ 
	   //客户查询按钮
		...
		//批量删除功能 
		 ...
		 //checkbox选择事件
		 $("#coupTable thead tr input[ id='dt_row_all_check']").live('click',function(e){
			 batchSendRule.selectCheckBox(e);
		 });
		 //checkbox选择事件
		 $("#custTable thead tr input[ id='dt_row_all_check']").live('click',function(e){
...
var batchSendRule={
		  ...
		  queryCust:function(){
					bDialog.open({
						...
						width : 820,
						height : 540,
						url : GLOBAL.WEBROOT + "/sendrule/custgrid",
	  ...
	  queryCoup:function(_shopId){
				bDialog.open({
					...
					width : 800,
					height : 540,
					url : GLOBAL.WEBROOT + "/sendrule/select",
					callback:function(data){
						   //确定 按钮_if_query==1
						   if(data && data.results && data.results[0]._if_query=='1'){
			...
			initCustList:function(_ids){
					...
					//ajax请求
					$.eAjax({
						url : GLOBAL.WEBROOT + "/sendrule/custList",
						data:[{name:'ids',value:_ids}],
						"dataType": "text",
		...
		initCoupList:function(_ids){
				...
				//ajax请求
				$.eAjax({
					url : GLOBAL.WEBROOT + "/sendrule/coupInfoList",
					data:[{name:'ids',value:_ids}],
					"dataType": "text",
					success : function(returnInfo) {
			 ...
			 //优惠券选择
			   ...
			   selectCheckBoxCust:function(e){
				   //选中 表示全选
				   if(e.currentTarget.checked){
					   $("#custTable tbody tr td input[ name='checkboxcust']").prop('checked',true);
				   }else{
					   //全部取消
					   $("#custTable tbody tr td input[ name='checkboxcust']").prop('checked',false);
				   }
			   },
			   //提交
			   save:function(objectBtn){
				      ...
				      if(_l.length<=0){
			     		...
			     		//ajax请求
			     		$.eAjax({
			     			url : GLOBAL.WEBROOT + "/sendrule/save",
			     			data:[{name:'custIds',value:_data1},{name:'coupIds',value:_data2}],
			     			success : function(returnInfo) {
			     				//成功返回
			     				if(returnInfo.resultFlag=='ok'){
			     					eDialog.alert("赠送成功！",function(){
			     						  window.location.href = GLOBAL.WEBROOT+'/sendrule';
			     					});
