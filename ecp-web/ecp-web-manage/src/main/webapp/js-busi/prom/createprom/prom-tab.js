   ...
   var promtab={
	  ...
	  goBack:function(_b){
		  ...
		  if(_backId=='copy' || _backId=='edit' ){
			  window.location.href = GLOBAL.WEBROOT+'/myprom';
			  return ;
		  }
		  //新增返回上一页
		  if(_backId=='create' || _backId==''||_backId==null || _backId=="undefined" ){
		      window.location.href = GLOBAL.WEBROOT+'/createprom/ct/'+$('#shopId').val();
		      return ;
		  }
		  history.go(-1);
	  },
	 		    		 ...
	 		    		 if(_l.length>0 ){
		 		    		  //商品选择
				 		    	  ...
				 		    	  for(var i=0;i<_l.length;i++){
				 		    		 ...
				 		    		 //促销量
				 		    		//gdsMapValue.push({'name':'promCnt','value':_promCnt});
 		   ...
 		   if(maxBuyCnt && minBuyCnt){
	 	    ...
	 	    var _discountData=this.getDiscountRuleData();
	 	      ...
	 	      $('#btnFormSubmit').attr('disabled',"true"); 
	     		//ajax请求
	     		$.eAjax({
	     			url : GLOBAL.WEBROOT + "/createprom/"+s,
	     			data : data,
  ...
  //保存
   ...
   //返回(编辑  修改 详情 新增 返回 页面不同)
	$('#btnReturnTab').click(function(){
		//window.location.href = GLOBAL.WEBROOT+'/createprom/ct/'+$(this).attr("shopId");
		//history.go(-1);
		 promtab.goBack();
	});
