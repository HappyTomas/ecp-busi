...
import com.zengshi.ecp.busi.buyer.vo.CustAddrRespVO;
	...
	public RSubmitOrderRespVO submitOrd(Model model,RSumbitMainsReqVO vo,HttpServletRequest request){
		...
		try{
	        ...
	        for(int i=0;i<ordMains.getPreOrdMainList().size();i++){
													...
													for (CoupSkuRespDTO coupSkuRespDTO : coupSkus) {
														for(RPreOrdSubResponse ordSub : ordMain.getPreOrdSubList()){
															if(ordSub.getSkuId().equals(coupSkuRespDTO.getSkuId())){
																Long discountFinalPrice = ordMains.getCartPromRespDTO().getCartPromItemDTOMap().get(ordSub.getCartItemId()).getDiscountFinalPrice().longValue();
				...
				LogUtil.info(MODULE, "sumbitMainVO:"+ JSON.toJSONString(sumbitMainVO));
				LogUtil.info(MODULE, "ordMain=====:"+ JSON.toJSONString(ordMain));
				//页面金额重计
			...
			resp.setAppkey(WxConstants.APPID);
