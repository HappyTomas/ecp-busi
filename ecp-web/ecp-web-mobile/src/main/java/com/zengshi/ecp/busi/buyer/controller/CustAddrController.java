    ...
    {   
        ...
        try {
            if(cusraddrDTO.getId() == null){
            	CustAddrReqDTO selReq = new CustAddrReqDTO();
            	selReq.setStaffId(selReq.getStaff().getId());
            	List<CustAddrResDTO> listaddr = custaddrRSV.listCustAddr(selReq);
            	if (listaddr != null && listaddr.size() >= 10) {
            		resultVo.setResultFlag(EcpBaseResponseVO.RESULT_FLAG_FAILURE);
            		resultVo.setResultMsg("亲~，收货地址不能超过10条记录。");
            		return resultVo;
            	}
            ...
            resultVo.setResultFlag(EcpBaseResponseVO.RESULT_FLAG_FAILURE);
            resultVo.setResultMsg("地址信息保存失败！");
        } catch (Exception e) {
            LogUtil.error(Module, "新增或编辑地址失败！", e);
            resultVo.setResultFlag(EcpBaseResponseVO.RESULT_FLAG_FAILURE);
            resultVo.setResultMsg("地址信息保存失败！");
