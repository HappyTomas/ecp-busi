 ...
 */
$(function(){
    var pre_url = GLOBAL.WEBROOT + '/order/my/';
    ...
    var operaIndex = {'00':1,'01':1,'02':0,'05':1,'80':2,'06':2,'07':1,'08':1,'99':1};
    ...
    var createOperaHtml = function(obj){
        ...
        if(status == "01") {
        	...
        	if(dispatchType!=''&&dispatchType!=2){
    ...
    var loadScroll = new LoadScroll("wrapper2", {
        url: GLOBAL.WEBROOT+'/order/my/scrollData',
        isAjax:true,
        call:loadCallBack,
        params:{
    ...
    $('body').delegate('.goexpresslist', 'click', function(e){
        //e.stopPropagation();
         ...
         location.href =  GLOBAL.WEBROOT+"/order/queryExpress/"+orderId;
    });
    ...
    $('body').delegate('.ordclick', 'click', function(e) {
        ...
        window.location.href = GLOBAL.WEBROOT + '/order/detail?orderId='+orderId;
        stopDefault(e);
    });
  ...
  //合并付款事件绑定=====start
	$("#allChecked").on('click',function(){
		if(this.checked){
			$("#wrapper2 .addCheck").each(function(){
	...
	$("#mergePay").on('click',function(){
		...
		$("#wrapper2 .addCheck").each(function(){
			...
			if(this.checked){
				if(c == 0){
					orderIds = _this.parents('.shopItem').find('input[name="orderId"]').val();
				}else{
					orderIds = orderIds + "," + _this.parents('.shopItem').find('input[name="orderId"]').val();
				}
				c ++;
			}
		});
