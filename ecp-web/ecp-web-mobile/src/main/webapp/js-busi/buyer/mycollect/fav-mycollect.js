$(function(){
	...
	//加载商品的
	$('#wrapper1').height($(window).height()-$('.am-header').height()-50);
    ...
    //加载店铺的
	$('#wrapper2').height($(window).height()-$('.am-header').height()-50);
...
var FavGoods = {
      ...
      addToCart : function(skuId,obj,gdsTypeId){
    	  ...
    	  var buyOnce = "";
    	  $.eAjax({
				url : GLOBAL.WEBROOT + "/favgoods/wetherbuyonce",
	  			data : {gdsTypeId:gdsTypeId},
	  			async : false,
      ...
      deleteOne : function (id){
    	  eDialog.confirm("确定要取消收藏吗？",function(){
    		 $.eAjax({
				...
				success : function(returnInfo) {
					if(returnInfo.resultFlag=='ok'){
						window.location.href = GLOBAL.WEBROOT + "/favgoods"; 
						 ...
						 new AmLoad({content:'取消收藏失败',type:'2'});
					 }
				 }
			 });
      	});
