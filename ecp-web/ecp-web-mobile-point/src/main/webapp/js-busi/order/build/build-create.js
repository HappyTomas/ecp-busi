$(function(){
	...
	$("#build_addr_save").click(function(e){
		...
		$.eAjax({
			url : GLOBAL.WEBROOT + "/point/order/build/saveaddr",
			data : val,
			datatype:'json',
				...
				if(data&&data.resultFlag == 'ok'){
					...
					if(id == ""){//新增
						...
						if($("#js_addrlist .js_addrunit").length == 1){
							...
							$("#addrId").val(data.id);
							//修改确认界面
							$("#build_custaddr").html($("#js_addrlist .js_addrunit").html());
							$("#build_custaddr").find(".adre-w").after('<span class="ui-arrow ui-arrow-right"></span>');
						}
	...
	$('div.js_build_coupon').click(function(e){
		...
		$(':checkbox:checked', $('.js_build_coupItemBean')).each(function () {
	...
	$('.js_build_coupItemBean').each(function(){
		...
		$(':checkbox',$(_this)).click(function(){
			...
			if(hasSelect){
				...
				var oldCoupShopValue = coupShop.val();
				if(checkSelect){
					coupShop.val(Number(oldCoupShopValue)+Number(coupValue));
				}else{
					coupShop.val(Number(oldCoupShopValue)-Number(coupValue));
				}
			}
	...
	$("#submitOrd").click(function(){
		...
		if(!$(obj).hasClass('mbtn-grad')){
			$.eAjax({
				url:GLOBAL.WEBROOT + '/point/order/build/submitOrd',
